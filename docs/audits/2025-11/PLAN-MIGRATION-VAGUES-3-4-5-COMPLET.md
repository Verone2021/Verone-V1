...
**Destination** : `@verone/utils` (ou rester en `apps/back-office/src/lib/` selon criticité)
...
**Option B - Garder en apps/back-office/src/lib/** (recommandé pour middleware/auth critiques Next.js)

- `middleware/*` → Rester en `apps/back-office/src/lib/middleware/`
- `auth/session-config.ts` → Rester en `apps/back-office/src/lib/auth/`
...
**Impact** : 267 occurrences dans `apps/back-office/src/app/`
...
| ---------------------------------- | -------- | ---------------- | -------------------- |
| `apps/back-office/src/app/`        | 117      | 604              | Script jscodeshift   |
| `apps/back-office/src/components/` | 205      | 155              | Script jscodeshift   |
| `apps/back-office/src/hooks/`      | 4        | 4                | Manuel (seulement 4) |
| **TOTAL**                          | **326**  | **763**          | -                    |
...
#### Dossier : `apps/back-office/src/lib/` (65 fichiers)
...
```bash
rm -rf apps/back-office/src/lib/
```
...
```bash
# Garder seulement
apps/back-office/src/lib/middleware/
apps/back-office/src/lib/auth/
# Supprimer le reste
```

**Gain espace estimé** : ~500 KB - 1 MB

#### Fichiers `apps/back-office/src/types/` obsolètes
...
- [ ] `apps/back-office/src/lib/` nettoyé (partiel ou complet)
- [ ] Fichiers obsolètes `apps/back-office/src/types/` supprimés
...
| Cleanup `apps/back-office/src/lib/`   | 50-65               | ~0.5-1 MB   |
| Cleanup `apps/back-office/src/types/` | 3-5                 | ~200 KB     |
...
| ----------- | ----------------------------------- | ----------- | ----------- | --------- | ---------- |
| **VAGUE 3** | Migration apps/back-office/src/lib/ | 65          | 77-106      | 4h        | 🟡 Moyenne |
...
- ✅ VAGUE 3 : Migration complète apps/back-office/src/lib/ (65 fichiers)
...
- [ ] Cleanup `apps/back-office/src/lib/` (Option A ou B)
- [ ] Cleanup `apps/back-office/src/types/` (fichiers obsolètes)
