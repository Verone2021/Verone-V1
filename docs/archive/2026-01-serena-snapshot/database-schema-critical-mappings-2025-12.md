# Mappings Critiques Base de Données Vérone

**Date création** : 2025-12-01
**Objectif** : Éviter les erreurs répétitives sur les noms de colonnes et relations FK
...
| Colonne attendue (FAUX) | Colonne réelle (CORRECT)   |
| ----------------------- | -------------------------- |
| `name`                  | `legal_name`               |
...
| Colonne attendue (FAUX) | Solution réelle (CORRECT)                           |
| ----------------------- | --------------------------------------------------- |
...
// Puis filtrer côté client : images.find(i => i.is_primary)?.public_url
```

**Alternative - Query séparée** :
...
## 4. Résumé des Relations Products

```typescript
// ✅ Query complète CORRECTE pour un produit avec toutes ses relations
const select = `
  id,
  sku,
  name,
...
Avant toute requête Supabase sur `products` :
