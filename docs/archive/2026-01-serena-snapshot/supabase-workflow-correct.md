# Workflow Supabase Correct - Mis à jour 2025-12-30
...
### 🗄️ Migrations via API Supabase (MÉTHODE RECOMMANDÉE)

```bash
# ✅ EXÉCUTER SQL DIRECTEMENT via API Supabase
curl -s -X POST "https://api.supabase.com/v1/projects/aorroydfjsrygmosnzrl/database/query" \
  ...
  -d '{"query": "VOTRE SQL ICI"}'

# Exemple pour appliquer une migration:
SQL=$(cat supabase/migrations/XXXXX.sql | tr '\n' ' ' | sed 's/"/\\"/g')
curl -s -X POST "https://api.supabase.com/v1/projects/aorroydfjsrygmosnzrl/database/query" \
  -H "Authorization: Bearer sbp_a38334ba650a3fe5820e1641e3d156c01528089b" \
  -H "Content-Type: application/json" \
  -d "{\"query\": \"$SQL\"}"
...
**RÈGLE ABSOLUE** : NE JAMAIS demander à l'utilisateur d'appliquer les migrations manuellement.
Toujours utiliser l'API Supabase directement avec le token d'accès ci-dessus.
