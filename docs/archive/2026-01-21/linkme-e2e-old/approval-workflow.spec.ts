...
import { loginLinkMe, loginBackOffice, CREDENTIALS } from './fixtures/auth';
import {
  getProductById,
  createTestProduct,
  cleanupTestData,
...
test.describe('Approval Workflow', () => {
  ...
  test('4.1: Submit product for approval', async ({ page }) => {
  ...
  test('4.2: Approve product from back-office', async ({ page }) => {
    ...
    await getProductById(productId); // Verify product exists first
  ...
  test('4.3: Reject product from back-office', async ({ page }) => {
    // Step 1: Create test product in pending_approval status
