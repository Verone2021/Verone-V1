# Profils Utilisateurs - Vérone

**Dernière mise à jour** : 2025-10-16
**Version** : 1.0
**Mainteneur** : Vérone Documentation Team

## Table des matières

- [Introduction](#introduction)
- [Structure Profil](#structure-profil)
- [Champs Metadata](#champs-metadata)
- [Relations Tenant](#relations-tenant)
- [Gestion Profils](#gestion-profils)

---

## Introduction

Documentation de la structure des profils utilisateurs dans le système Vérone : champs, metadata, relations avec tenants et rôles.

**À documenter** :
- Schéma table `user_profiles`
- Champs obligatoires vs optionnels
- Metadata JSON structure
- Logique liaison tenant_id
- Workflows création/modification profils

---

**Retour** : [Documentation Authentification](/Users/romeodossantos/verone-back-office-V1/docs/auth/README.md) | [Index Principal](/Users/romeodossantos/verone-back-office-V1/docs/README.md)
