...
// Exemples
const tabouret = { price_ht: 75.0 };
const pack4 = { base_quantity: 4, discount_rate: 0.1 };
...
const palettePack = {
  base_quantity: 50,
  unit_price_ht: 65.0, // Au lieu de 75.00
...
function validatePackage(
  ...
  // Remise cohérente
  if (package.discount_rate && package.discount_rate > 0.4) {
...
function generateCustomLabels(product: Product): string[] {
  ...
  const maxDiscount = Math.max(
    ...product.packages.map(p => p.discount_rate || 0)
  );
  if (maxDiscount > 0) {
...
const dynamicRules = [
  { min_quantity: 1, discount_rate: 0.0 }, // 1-4: prix normal
  { min_quantity: 5, discount_rate: 0.1 }, // 5-19: -10%
  { min_quantity: 20, discount_rate: 0.2 }, // 20-49: -20%
  { min_quantity: 50, discount_rate: 0.3 }, // 50+: -30%
];
```
