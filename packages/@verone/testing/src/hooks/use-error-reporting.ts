...
export function useErrorReporting(options: UseErrorReportingOptions = {}) {
  ...
  const initializeErrorDetection = useCallback(() => {
    ...
    const handleWindowError = (event: ErrorEvent) => {
      ...
      setConsoleErrors(prev =>
        [`[${new Date().toISOString()}] ${errorInfo}`, ...prev].slice(0, 50)
      );
    };
  ...
  const getPerformanceMetrics = useCallback(():
    ...
    return {
      ...
      largestContentfulPaint: largestContentfulPaint
        ? largestContentfulPaint.startTime
        : 0,
  ...
  const uploadScreenshot = useCallback(
    async (file: File, reportId: string): Promise<string | null> => {
      try {
        // Vérifier la taille du fichier
        if (file.size > maxScreenshotSize) {
          throw new Error(
            `Screenshot trop volumineux: ${Math.round(file.size / 1024 / 1024)}MB > ${Math.round(maxScreenshotSize / 1024 / 1024)}MB`
          );
        }
        ...
        const {
          data: { publicUrl },
        } = supabase.storage.from('test-screenshots').getPublicUrl(fileName);
  ...
  const submitReport = useCallback(
    async (
      report: Omit<
        ErrorReport,
        'id' | 'browserInfo' | 'createdAt' | 'reportedBy'
      >
    ): Promise<string | null> => {
  ...
  const loadReports = useCallback(
    async (testId?: string): Promise<ErrorReport[]> => {
      try {
        let query = supabase
          .from('test_error_reports' as any)
          .select('*')
        ...
        const formattedReports: ErrorReport[] = (data || []).map(
          (report: any) => ({
            id: report.id,
            ...
            screenshotUrls: report.screenshot_url
  ...
  return {
    ...
    getReportsByTest: (testId: string) =>
      reports.filter(r => r.testId === testId),
