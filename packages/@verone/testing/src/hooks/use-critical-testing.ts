...
export function useCriticalTesting(options: UseCriticalTestingOptions = {}) {
   ...
   */
  const runCriticalTests = useCallback(
    async (module?: TestModule, priorityOnly: boolean = false) => {
      ...
      try {
        ...
        console.log(
          `🚀 CRITICAL TESTING 2025: Running ${testsToRun.length} tests`
        );
        ...
        console.log(
          `✅ CRITICAL TESTING 2025: Completed ${testsToRun.length} tests`
        );
      } catch (error: any) {
        setState(prev => ({
          ...prev,
  ...
  const runConsoleErrorCheck = useCallback(async () => {
    ...
    try {
      ...
      const consoleMessages =
        (await (window as any).mcp?.playwright?.browser_console_messages?.()) ||
        [];
  ...
  const runPerformanceCheck = useCallback(
    async (module: TestModule) => {
      ...
      const performanceResult = await simulatePerformanceCheck(
        module,
        sloTargets[module]
      );
