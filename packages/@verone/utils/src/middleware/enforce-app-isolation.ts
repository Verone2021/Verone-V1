 ...
 *
 * Replaces the old app-isolation.ts (which relied on user_profiles.app_source).
 *
 ...
 * 3. Session but no role in user_app_roles for this app → sign out + redirect with error
 * 4. Session + role → pass through (with refreshed cookies)
 */
export async function enforceAppIsolation(
  ...
  // --- Authenticated → verify role for this app ---
