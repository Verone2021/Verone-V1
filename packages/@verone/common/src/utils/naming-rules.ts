 ...
 */
function generateCleanVariantName(
  variantName: string,
  baseName: string
): string {
  // Retirer le nom de base du début
  const cleanName = variantName
    .replace(new RegExp(`^${baseName}\\s*`, 'i'), '')
    .trim();
 ...
 */
export function sortVariantSiblings(
  variants: Product[],
...
export function generateVariantName(
  ...
  const matiere = findAttributeValue(
    variantAttributes,
    'matiere',
    'material',
    'matière'
  );
  const taille = findAttributeValue(
    variantAttributes,
    'taille',
    'size',
    'dimension'
