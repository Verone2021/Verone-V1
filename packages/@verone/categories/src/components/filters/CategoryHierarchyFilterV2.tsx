...
export function CategoryHierarchyFilterV2({
  ...
  const enrichedHierarchy = useMemo(() => {
    ...
    const enrichedFamilies: EnrichedFamily[] = families
      .map(family => {
    ...
    return selectedSubcategories
      .map(subId => {
        ...
        const family = category
          ? families.find(f => f.id === category.family_id)
          : null;
            ...
            {activeFilters.map(filter => (
              ...
              >
                <span className="text-xs">
                  {filter.familyName} › {filter.categoryName} ›{' '}
      ...
      <div className="border border-gray-200 rounded-lg bg-white overflow-hidden">
        <div className="max-h-96 overflow-y-auto p-2 space-y-1">
          {enrichedHierarchy.map(family => {
            ...
            const selectedCountInFamily = family.categories.reduce(
              (sum, cat) => {
                return (
                  sum +
