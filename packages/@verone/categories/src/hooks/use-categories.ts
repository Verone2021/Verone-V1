    ...
    try {
      ...
      if (error) {
        ...
        const { data: categoriesData, error: categoriesError } = await supabase
          .from('categories')
          .select(
            'id, name, slug, level, family_id, display_order, is_active, description, image_url, cloudflare_image_id, google_category_id, facebook_category, meta_title, meta_description, is_visible_menu, retail_coefficient, wholesale_coefficient, created_at, updated_at'
          )
          .order('level')
          .order('display_order');
  ...
  const createCategory = async (
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(
          'id, name, slug, level, family_id, display_order, is_active, description, image_url, created_at, updated_at'
        )
        .single();
