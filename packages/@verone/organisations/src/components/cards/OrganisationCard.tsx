...
export function OrganisationCard({
  ...
  const typeInfo = getOrganisationTypeInfo(
    organisation.type,
        ...
        {organisation.type === 'supplier' && organisation.supplier_segment ? (
          <div className="flex flex-wrap items-center gap-1">
            <SupplierSegmentBadge
              segment={organisation.supplier_segment}
              size="sm"
              showIcon
            />
        ...
        {(countryCode || organisation.city) && (
          <div className="space-y-0.5">
            {countryCode && (
              <Badge
                variant="outline"
                className="text-[10px] px-1.5 py-0.5 leading-tight border-gray-300"
              >
                🌍 {countryCode}
              </Badge>
            )}
            ...
            <>
              <Link href={`${baseUrl}/${organisation.id}`} className="flex-1">
                <ButtonV2
                  variant="ghost"
                  size="sm"
                  className="w-full text-[11px] h-7 leading-tight"
                >
                  Détails
                </ButtonV2>
              </Link>
