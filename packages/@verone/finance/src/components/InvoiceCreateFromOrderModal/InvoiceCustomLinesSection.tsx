...
interface IInvoiceCustomLinesSectionProps {
...
export function InvoiceCustomLinesSection({
  customLines,
  setCustomLines,
  showAddLine,
  setShowAddLine,
  ...
  newLineVatRate,
  setNewLineVatRate,
}: IInvoiceCustomLinesSectionProps): React.ReactNode {
  ...
  const handleRemoveLine = (lineId: string): void => {
    setCustomLines(customLines.filter(l => l.id !== lineId));
  };
