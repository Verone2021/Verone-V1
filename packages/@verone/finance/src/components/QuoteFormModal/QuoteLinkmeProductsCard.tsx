...
interface QuoteLinkmeProductsCardProps {
  selectionName: string;
  items: SelectionItem[];
  addedProductIds: (string | null)[];
  onAddProduct: (item: SelectionItem) => void;
}

export function QuoteLinkmeProductsCard({
  selectionName,
  items,
  addedProductIds,
  onAddProduct,
}: QuoteLinkmeProductsCardProps) {
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base">
          Produits de la sélection — {selectionName}
        </CardTitle>
      </CardHeader>
      <CardContent>
        {items.length === 0 ? (
          ...
          <div className="space-y-2 max-h-[250px] overflow-y-auto">
            {items.map(selItem => {
              ...
              const alreadyAdded = addedProductIds.includes(selItem.product_id);
