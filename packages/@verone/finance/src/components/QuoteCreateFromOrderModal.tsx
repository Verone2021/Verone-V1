...
export function QuoteCreateFromOrderModal({
  ...
  // Champs devis : date d'émission, label, validité
  ...
  // Prochain numéro de devis (approximatif)
  ...
  // Récupérer le prochain numéro de devis au chargement
  useEffect(() => {
    if (!open) return;
    setLoadingNextNumber(true);
    void fetch('/api/qonto/quotes?per_page=1&sort_by=number:desc')
      .then(res => res.json())
      .then(data => {
        if (data.success && data.quotes?.[0]?.quote_number) {
          const lastNumber = data.quotes[0].quote_number as string;
          ...
          if (match) {
            ...
            setNextQuoteNumber(`${prefix}${nextNum}`);
          } else {
  ...
  const handleSendEmail = async (): Promise<void> => {
    if (!createdQuote?.id || !order) return;
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {status === 'success' ? 'Devis créé' : 'Générer un devis'}
          </DialogTitle>
          <DialogDescription>
            {status === 'success'
        ...
        <div className="flex-1 overflow-y-auto pr-2">
          {status === 'success' && createdQuote ? (
            ...
            <div className="space-y-4">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <Label
                      htmlFor="quote-different-shipping"
              ...
              {/* Date de devis + Label + Validité + Prochain numéro */}
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm flex items-center gap-2">
                ...
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        placeholder="Ex: Devis mobilier Pokawa"
                        className="h-8"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/50 p-2 rounded">
                    ...
                    {loadingNextNumber ? (
                      ...
                      <span>
                        Prochain numéro : <strong>{nextQuoteNumber}</strong>{' '}
                        (approximatif, attribué par Qonto)
                      </span>
                    ) : (
                      <span>
                        Le numéro de devis sera attribué par Qonto à la création
                      </span>
                    )}
                  </div>
                </CardContent>
              ...
              {isMissingSiret && (
                <div className="rounded-lg border border-red-200 bg-red-50 p-3 space-y-2">
                  <div className="flex items-center gap-2 text-sm font-medium text-red-800">
                    ...
                    SIRET ou n° TVA requis pour un devis organisation
                  </div>
