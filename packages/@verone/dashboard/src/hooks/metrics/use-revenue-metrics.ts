...
export function useRevenueMetrics() {
  ...
  const fetch = async (): Promise<RevenueMetrics> => {
    try {
      ...
      const { data: previousMonthOrders, error: previousMonthError } =
        await supabase
          .from('sales_orders')
          .select('total_ht')
          .in('status', [
      ...
      const today = (todayOrders || []).reduce(
        (sum, order) => sum + parseFloat(String(order.total_ht || 0)),
        0
      );
      const month = (monthOrders || []).reduce(
        (sum, order) => sum + parseFloat(String(order.total_ht || 0)),
        0
      );
      const year = (yearOrders || []).reduce(
        (sum, order) => sum + parseFloat(String(order.total_ht || 0)),
        0
      );
      const previousMonth = (previousMonthOrders || []).reduce(
        (sum, order) => sum + parseFloat(String(order.total_ht || 0)),
        0
      );
