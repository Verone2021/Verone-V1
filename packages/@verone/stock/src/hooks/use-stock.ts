...
export function useStock() {
  ...
  const getProductStockAdvanced = useCallback(
  ...
  const createManualMovement = useCallback(
    async (movementData: ManualMovementData) => {
      setLoading(true);
      try {
        ...
        switch (movementData.movement_type) {
          ...
          case 'remove':
            ...
            if (newQuantity < 0) {
              throw new Error(
                `Stock insuffisant. Disponible: ${currentStock}, Demandé: ${movementData.quantity}`
              );
            }
            break;
        ...
        const dbMovementType =
          movementData.movement_type === 'add'
            ? 'IN'
            : movementData.movement_type === 'remove'
              ? 'OUT'
              : 'ADJUST';
        ...
        if (movementError) {
          ...
          if (movementError.message?.includes('Incohérence dans le calcul')) {
            ...
            movementError.message?.includes('ne peut pas être négatif')
          ) {
            ...
            throw new Error(
              'Permissions insuffisantes pour créer un mouvement de stock'
            );
          } else if (
            movementError.code === 'PGRST301' ||
            movementError.code === '42501'
          ) {
            throw new Error(
  ...
  const createForecastMovement = useCallback(
    async (
      productId: string,
      quantity: number,
      type: 'in' | 'out',
    ...
    return stockData
      ...
      .map(item => ({
        ...
        alert_type:
          item.stock_real <= 0
            ? 'critical'
            : item.stock_real <= item.min_stock
              ? 'low'
              : 'forecast',
        alert_message:
          item.stock_real <= 0
  ...
  const findProductsByStock = useCallback(
    (filters: {
      minReal?: number;
      maxReal?: number;
      ...
      return stockData.filter(item => {
        if (filters.minReal !== undefined && item.stock_real < filters.minReal)
          return false;
        if (filters.maxReal !== undefined && item.stock_real > filters.maxReal)
          return false;
        if (
          filters.hasForecasted &&
          item.stock_forecasted_in + item.stock_forecasted_out === 0
        )
          return false;
