...
export function MovementDetailsModal({
  ...
  const getMovementTypeBadge = (movementType: string) => {
    switch (movementType) {
      case 'IN':
        return (
          <Badge className="bg-green-100 text-green-800">Entrée de Stock</Badge>
        );
      case 'OUT':
  ...
  const getMovementOrigin = () => {
    ...
    if (
      ...
      const orderType = referenceType.includes('purchase')
        ? 'ACHAT'
        : referenceType.includes('sale')
          ? 'VENTE'
          : 'COMMANDE';
          ...
          {movement.unit_cost && (
            <div>
              ...
              <div className="mt-2 p-2 bg-gray-50 rounded text-sm">
                ...
                <span className="font-medium">
                  {formatPrice(
                    movement.unit_cost * Math.abs(movement.quantity_change)
                  )}
                </span>
              </div>
            </div>
            ...
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="text-sm">
                ...
                <span className="text-blue-700">
                  Ce mouvement affecte les prévisions de stock (
                  {movement.forecast_type === 'in' ? 'entrée' : 'sortie'}{' '}
                  prévue)
                </span>
              </div>
            </div>
