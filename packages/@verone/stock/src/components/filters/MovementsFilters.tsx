...
export function MovementsFilters({
  ...
  const [localFilters, setLocalFilters] =
    useState<MovementHistoryFilters>(filters);
  ...
  useEffect(() => {
    const loadUsers = async () => {
      ...
      try {
        ...
        const userIds = [
          ...new Set(movementUsers.map(m => m.performed_by).filter(Boolean)),
        ];
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              className={
                localFilters.dateRange &&
                new Date(localFilters.dateRange.from).toDateString() ===
        ...
        <div className="space-y-3">
          <Label className="flex items-center justify-between">
            <span>Motifs</span>
            {localFilters.reasonCodes &&
              localFilters.reasonCodes.length > 0 && (
                <Badge variant="secondary" className="ml-2">
                  {localFilters.reasonCodes.length} sélectionné
                  {localFilters.reasonCodes.length > 1 ? 's' : ''}
                </Badge>
              )}
          </Label>
