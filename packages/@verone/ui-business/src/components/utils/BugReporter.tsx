...
import { useSupabaseMutation } from '@verone/common/hooks';
import { useUserActivityTracker } from '@verone/notifications/hooks';
...
export function BugReporter({ trigger, onSubmitSuccess }: BugReporterProps) {
  ...
  const uploadScreenshotMutation = useSupabaseMutation<string>(
    async (
      supabase,
      { screenshot, filename }: { screenshot: string; filename: string }
    ) => {
    ...
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (screenshot) {
          ...
          screenshotUrl =
            (await uploadScreenshotMutation.mutate({ screenshot, filename })) ||
            undefined;
        }
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            Aidez-nous à améliorer Vérone en signalant les problèmes que vous
            rencontrez.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={e =>
                  setFormData({ ...formData, actual_behavior: e.target.value })
                }
          ...
          {consoleErrors.length > 0 && (
            <Alert>
              ...
              <AlertDescription>
                <strong>{consoleErrors.length} erreur(s) détectée(s)</strong>{' '}
                dans la console. Elles seront automatiquement incluses dans le
                rapport.
              </AlertDescription>
            </Alert>
          )}
