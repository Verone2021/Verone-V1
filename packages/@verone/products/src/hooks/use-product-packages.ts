...
export function useProductPackages({
  ...
  const fetchPackages = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(
          'id, product_id, quantity, unit, unit_price, display_order, is_active, created_at, updated_at'
        )
  ...
  const calculatePackagePrice = useCallback(
    (basePrice: number, packageData: ProductPackage): number => {
  ...
  return {
    ...
    isValidPackageSystem:
      packages.length > 0 &&
      packages.some(pkg => pkg.is_default && pkg.type === 'single'),
  };
}
