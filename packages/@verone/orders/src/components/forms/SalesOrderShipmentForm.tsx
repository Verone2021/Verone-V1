...
export function SalesOrderShipmentForm({
  ...
  const [carrierType, setCarrierType] = useState<
    'packlink' | 'mondial_relay' | 'chronotruck' | 'other'
  >('packlink');
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Commande {(salesOrder as any).so_number || salesOrder.order_number}{' '}
            •{' '}
          ...
          <Card className="p-4">
            <div className="space-y-4">
              <div>
                ...
                <Select
                  value={carrierType}
                  onValueChange={(v: any) => setCarrierType(v)}
                >
        ...
        <Card className="p-4">
          ...
          <div className="space-y-2">
            {history.map((h, idx) => (
              <div
                key={idx}
                className="text-sm border-l-2 border-verone-primary pl-3 py-1"
              >
                <div className="font-medium">
                  {formatDate(h.shipped_at)} • {h.carrier_name} •{' '}
                  {h.total_quantity} unités
                </div>
                {h.tracking_number && (
                  <div className="text-xs text-muted-foreground">
