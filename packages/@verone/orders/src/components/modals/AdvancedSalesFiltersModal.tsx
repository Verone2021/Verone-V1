...
const STATUS_OPTIONS = [
  ...
  { value: 'partially_shipped', label: 'Part. expédiée' },
...
const CUSTOMER_TYPE_OPTIONS = [
  ...
  { value: 'professional', label: 'Professionnels (tous)' },
  { value: 'organisation', label: 'Organisations (indép.)' },
...
interface AdvancedSalesFiltersModalProps {
  filters: SalesAdvancedFilters;
  onApply: (filters: SalesAdvancedFilters) => void;
}

export function AdvancedSalesFiltersModal({
  filters,
  onApply,
}: AdvancedSalesFiltersModalProps) {
  const [open, setOpen] = useState(false);
  const [local, setLocal] = useState<SalesAdvancedFilters>(filters);

  const { enseignes } = useActiveEnseignes();

  const activeCount = useMemo(
    () => countActiveFilters(filters, DEFAULT_SALES_FILTERS),
    [filters]
  );
  ...
  const handleReset = () => {
    setLocal({ ...DEFAULT_SALES_FILTERS });
  };
    ...
    <Dialog open={open} onOpenChange={handleOpen}>
      ...
      <DialogContent className="sm:max-w-[520px] max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          ...
          {/* Section: Type client + Enseigne */}
          <div className="space-y-3">
            <Label className="text-sm font-semibold">
              Type client / Enseigne
            </Label>
            <Select
            ...
            {/* Sélecteur enseigne spécifique */}
            {local.customerType === 'enseigne' && enseignes.length > 0 && (
              <Select
                value={local.enseigneId ?? 'all'}
                onValueChange={value =>
                  setLocal(prev => ({
                    ...prev,
          ...
          {/* Section: Montant TTC */}
          <div className="space-y-3">
            <Label className="text-sm font-semibold">Montant TTC (€)</Label>
