 ...
 * Sends notification email to Verone back-office when an Enseigne order is submitted
 *
 * Mirrors: apps/back-office/src/app/api/emails/notify-enseigne-order/route.ts
 */
...
interface NotifyEnseigneOrderRequest {
  ...
  totalTtc: number;
  source: 'client' | 'affiliate' | 'unified_form';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const sourceLabels: Record<string, string> = {
      client: 'Client (via s\u00e9lection publique)',
      affiliate: 'Affili\u00e9 (via back-office)',
      unified_form: 'Formulaire unifi\u00e9 (LinkMe)',
    };
    ...
    const appUrl =
      process.env.NEXT_PUBLIC_BACK_OFFICE_URL ?? 'https://app.verone.fr';
