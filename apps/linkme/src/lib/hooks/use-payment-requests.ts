    ...
    queryFn: async (): Promise<PaymentRequestWithCommissions | null> => {
      ...
      const { data: request, error: requestError } = await supabase
        ...
        .select(
          ...
          tax_rate, status, invoice_received, financial_document_id,
          invoice_file_url, invoice_file_name,
  ...
  return useMutation({
    mutationFn: async (
      ...
      const { data: request, error: createError } = await supabase
        ...
        .select(
          'id, affiliate_id, request_number, total_amount_ht, total_amount_ttc, tax_rate, status, invoice_received, financial_document_id, invoice_file_url, invoice_file_name, invoice_received_at, paid_at, paid_by, payment_reference, payment_proof_url, notes, created_at, updated_at'
        )
        .single();
...
export function useUploadInvoice() {
  ...
  return useMutation({
    mutationFn: async (input: UploadInvoiceInput): Promise<string> => {
      ...
      const filePath = `${requestId}/${timestamp}.pdf`;
      ...
      if (uploadError) {
        console.error('[useUploadInvoice] Erreur upload facture:', uploadError);
        throw new Error("Erreur lors de l'upload du fichier");
      }

      // On stocke le path (pas une URL publique qui n'existe plus)
      // La signed URL est générée à la demande via /api/profil/facturation/invoice/[requestId]/url
      ...
      if (updateError) {
        console.error(
          '[useUploadInvoice] Erreur mise à jour demande:',
          updateError
        );
  ...
  return useMutation({
    mutationFn: async (requestId: string): Promise<void> => {
      const { error } = await supabase
        ...
        .eq('id', requestId)
        .in('status', ['pending', 'partially_paid']); // Seulement si pas encore payé
...
function mapPaymentRequest(data: Record<string, unknown>): PaymentRequest {
  return {
    ...
    invoiceReceived: (data.invoice_received as boolean) ?? false,
    financialDocumentId: (data.financial_document_id as string) || null,
