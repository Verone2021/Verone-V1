...
export function SelectionsSection() {
  ...
  async function handleCreateSelection() {
    if (!formData.affiliate_id || !formData.name.trim() || selectedProducts.length === 0) {
      toast({
        title: 'Erreur',
        description: 'Veuillez remplir tous les champs et sélectionner au moins un produit',
        variant: 'destructive',
      });
      return;
    }
          ...
          <Table>
            ...
            <TableBody>
              {filteredSelections.map(selection => {
                const statusInfo = statusConfig[(selection.status || 'draft') as keyof typeof statusConfig];
    ...
    <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Créez une mini-boutique pour un affilié avec une sélection de produits
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid gap-2">
            ...
            <Select
              ...
              onValueChange={value => setFormData(prev => ({ ...prev, affiliate_id: value }))}
            >
              <SelectTrigger>
          ...
          <div className="grid gap-2">
            ...
            <Select
              ...
              onValueChange={value => setFormData(prev => ({ ...prev, status: value as 'draft' | 'active' }))}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="grid gap-2">
            ...
            {selectedProducts.length > 0 && (
              <div className="border rounded-md p-3 bg-blue-50 space-y-2">
                <p className="text-sm font-medium text-blue-800">Produits dans la sélection :</p>
                {selectedProducts.map(product => (
                  <div key={product.product_id} className="flex items-center justify-between bg-white p-2 rounded border">
                    <div className="flex-1">
                      <p className="text-sm font-medium">{product.product_name}</p>
                      <p className="text-xs text-muted-foreground">
                        Prix: {product.base_price_ht.toFixed(2)}€ HT |
                        Marge: {product.margin_rate}% |
                        PV: {(product.base_price_ht * (1 + product.margin_rate / 100)).toFixed(2)}€
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        onChange={e => updateProductMargin(product.product_id, Number(e.target.value))}
                      ...
                      <ButtonV2
                        variant="ghost"
                        size="sm"
                        onClick={() => removeProductFromSelection(product.product_id)}
                      >
            ...
            <div className="max-h-48 overflow-y-auto border rounded-md">
              {filteredCatalogProducts.length === 0 ? (
                <div className="p-4 text-center text-muted-foreground text-sm">
                  {productSearch ? 'Aucun produit trouvé' : 'Chargement du catalogue...'}
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Réf: {product.product_reference} | Prix: {product.product_price_ht.toFixed(2)}€ | Marge suggérée: {product.suggested_margin_rate}%
                          </p>
                        </div>
                        {isSelected ? (
        ...
        <DialogFooter>
          ...
          <ButtonV2
            ...
            disabled={saving || !formData.affiliate_id || !formData.name.trim() || selectedProducts.length === 0}
          >
            {saving ? 'Création...' : `Créer (${selectedProducts.length} produit${selectedProducts.length > 1 ? 's' : ''})`}
          </ButtonV2>
        </DialogFooter>
      </DialogContent>
    </Dialog>
