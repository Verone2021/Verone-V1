...
const isOrderEditable = (order: SalesOrder) => {
  // Seules les commandes sans canal spécifique sont éditables
  return (
    !order.channel_id ||
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          ...
          <SalesOrderFormModal
            buttonLabel="Nouvelle commande manuelle"
            onSuccess={() => {
              fetchOrders();
              fetchStats();
            }}
                        ...
                        <TableRow>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              {/* Lien externe pour commandes de canaux (LinkMe, Site Internet) */}
                              {getChannelRedirectUrl(order) && (
                                <Link
                                ...
                                >
                                  <IconButton
                                    ...
                                    variant="outline"
                                    size="sm"
                                    label={`Gérer dans ${order.sales_channel?.name || 'le CMS du canal'}`}
                                  />
                                </Link>
                              )}

                              {/* Modifier (draft ou validated) - Désactivé pour commandes de canaux */}
                              {(order.status === 'draft' ||
                              ...
                              {/* Dévalider (validated uniquement) */}
                              {order.status === 'validated' && (
                                <IconButton
                                  icon={RotateCcw}
                                  variant="outline"
                              ...
                              {/* Expédier (validated ou partially_shipped) */}
                              {(order.status === 'validated' ||
                                order.status === 'partially_shipped') && (
                                <IconButton
                                  icon={Truck}
                              ...
                              {/* Annuler disabled pour validated - Doit dévalider d'abord */}
                              {order.status === 'validated' && (
                                <IconButton
                                  icon={Ban}
                                  variant="outline"
