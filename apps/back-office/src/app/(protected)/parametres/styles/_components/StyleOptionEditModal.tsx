...
interface StyleOptionEditModalProps {
  item: StyleOption | null;
  open: boolean;
  onClose: () => void;
  onSaved: () => void;
  onCreate: (input: {
...
export function StyleOptionEditModal({
  item,
  open,
  onClose,
  onSaved,
  ...
  return (
    ...
    >
      <DialogContent className="h-screen md:h-auto md:max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {isNew ? 'Créer un style' : `Éditer · ${item?.label}`}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto md:max-h-[70vh] space-y-4 px-1">
          ...
          <div className="flex items-center justify-between rounded border border-gray-200 p-3">
            <div>
              <Label className="text-sm">Style actif</Label>
              <p className="text-xs text-gray-500 mt-0.5">
                Un style inactif n'apparaît plus dans les formulaires ni dans
        ...
        <DialogFooter className="flex-col gap-2 md:flex-row">
          ...
          <Button
            ...
            onClick={() => {
              void save().catch(err => {
                console.error('[StyleOptionEditModal] save failed:', err);
              });
            }}
          >
