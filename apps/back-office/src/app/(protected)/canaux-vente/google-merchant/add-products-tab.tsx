...
import { useAddProductsToGoogleMerchant } from '@verone/channels';
...
export function AddProductsTab() {
  const { data: gmProducts } = useGoogleMerchantProducts();
  const addProducts = useAddProductsToGoogleMerchant();
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [search, setSearch] = useState('');

  const { data: allProducts, isLoading } = useQuery<EligibleProduct[]>({
    queryKey: ['gm-eligible-products'],
    queryFn: async () => {
  ...
  const syncedIds = useMemo(
    () => new Set((gmProducts ?? []).map(p => p.product_id)),
    [gmProducts]
  );
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const count = result.data?.successCount ?? ids.length;
      toast.success(`${count} produit(s) ajoute(s) a Google Merchant`);
      setSelectedIds(new Set());
    } catch {
    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-muted-foreground">
          Tous les produits du site internet sont deja sur Google Merchant !
        </p>
      </div>
    );
  }
