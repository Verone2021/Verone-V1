...
function InfoRequestItem({ req }: { req: InfoRequest }) {
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1 flex-wrap">
          ...
          <Badge variant="outline" className="text-[9px] px-1 py-0">
            {req.recipient_type === 'requester' ? 'Demandeur' : 'Proprietaire'}
          </Badge>
        </div>
        <div className="text-gray-500 mt-0.5">
          ...
          {isCompleted && req.completed_at && (
            <span className="text-green-700 ml-1">
              — Complete{' '}
              {new Date(req.completed_at).toLocaleDateString('fr-FR', {
                day: 'numeric',
                month: 'short',
              })}
            </span>
          ...
          {isCancelled && req.cancelled_reason && (
            <span className="text-red-700 ml-1">— {req.cancelled_reason}</span>
          )}
        </div>
      </div>
    </div>
      ...
      <CardContent className="p-3">
        ...
        <div className="space-y-2">
          {[...infoRequests]
            ...
            .map(req => (
              <InfoRequestItem key={req.id} req={req} />
            ))}
        </div>
      </CardContent>
    </Card>
