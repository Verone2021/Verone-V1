...
/**
 * Page Performance Globale LinkMe
 *
 * Refonte 2025-12-17:
 * - Bouton retour vers Vue d'ensemble
 * - Filtres Année + Période
 * - KPIs compacts
 *
 * Vue d'ensemble des performances de tous les affiliés:
 * - Panier moyen
 * - CA généré (HT)
 * - Commissions (TTC)
 * - Nb commandes
 * - Top 10 produits
 * - Liste affiliés avec drill-down
 *
...
function AffiliateRow({ affiliate }: AffiliateRowProps) {
  return (
    <Link
      href={`/canaux-vente/linkme/analytics/performance/${affiliate.id}`}
    ...
    >
      <div className="flex h-9 w-9 items-center justify-center rounded-full bg-blue-100 text-blue-600 font-semibold text-sm shrink-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-500">
          {affiliate.ordersCount} commande{affiliate.ordersCount > 1 ? 's' : ''}
        </p>
      </div>
      ...
      <div className="flex items-center gap-4 text-right shrink-0">
        <div>
          <p className="text-sm font-semibold text-gray-900">
            {formatCurrency(affiliate.totalRevenueHT)}
          </p>
          <p className="text-xs text-gray-500">CA HT</p>
        </div>
        <div>
          <p className="text-sm font-semibold text-emerald-600">
            {formatCurrency(affiliate.totalCommissionsTTC)}
          </p>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="sticky top-0 z-10 border-b bg-white px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {/* Back Button */}
            <Link href="/canaux-vente/linkme/analytics">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Métriques de vente LinkMe</p>
            </div>
          </div>
      ...
      <div className="flex-1 p-6 overflow-auto space-y-6">
        ...
        {/* Two columns: Top Products + Affiliates */}
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div>
                  ...
                  {data.affiliates.length > 10 && (
                    <div className="pt-3 text-center">
                      <Badge variant="outline" className="text-gray-500">
                        +{data.affiliates.length - 10} autres affiliés
                      </Badge>
                    </div>
                  )}
                </div>
