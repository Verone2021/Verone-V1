...
export default function SourcingValidationPage() {
  ...
  const { products, loading, validateSourcing } = useSourcingProducts({});

  const [selectedProduct, setSelectedProduct] =
    useState<SourcingProduct | null>(null);
  const [validationNotes, setValidationNotes] = useState('');
  const [activeTab, setActiveTab] = useState<'validation' | 'samples'>(
    'validation'
  );
  ...
  const productsReadyForCatalog = products.filter(
    p => p.supplier_id && p.status === 'sourcing' && !p.requires_sample
  );
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-black">
                Validation Produits Sourcing
              </h1>
              <p className="text-gray-600 mt-1">
                Workflow complet: Sourcing → Échantillons → Catalogue
              </p>
            </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
          ...
          <Card className="border-black">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-black">
                {productsRequiringSamples.length}
              </div>
          ...
          <Card className="border-black">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-black">
                {productsReadyForCatalog.length}
              </div>
        ...
        {activeTab === 'validation' ? (
          ...
          <Card className="border-black">
            <CardHeader>
              ...
              <CardDescription>
                Produits sourcing à valider avant passage échantillons/catalogue
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {productsToValidate.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-gray-500">
                      Tous les produits sourcés ont été traités
                    </p>
                  </div>
                ) : (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 text-sm">
                        ...
                        <div>
                          ...
                          <span className="font-medium text-black ml-1">
                            {(product as any).sourcing_type === 'client'
                              ? 'Client spécifique'
                              : 'Catalogue général'}
                          </span>
                        </div>
                        <div>
                          ...
                          <span
                            className={`font-medium ml-1 ${product.requires_sample ? 'text-black' : 'text-green-600'}`}
                          >
          ...
          <Card className="border-green-500 mt-6">
            <CardHeader>
              <CardTitle className="text-green-700">
                Prêts pour Catalogue ({productsReadyForCatalog.length})
              </CardTitle>
              <CardDescription>
                Produits validés prêts pour transfert au catalogue
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          >
            <DialogContent className="max-w-lg">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <Label htmlFor="estimated_selling_price">
                ...
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={e =>
                      setValidationData(prev => ({
                        ...prev,
                        requires_sample: e.target.checked,
                      }))
                    }
                    className="rounded border-gray-300"
                  />
                  <Label htmlFor="requires_sample">
                    Nécessite validation d'échantillons
                  </Label>
                </div>
                <Textarea
