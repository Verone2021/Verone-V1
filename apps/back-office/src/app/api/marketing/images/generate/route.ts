...
import { getBrand } from '@verone/marketing/data';
import { getPreset } from '@verone/marketing/data';
...
const MAX_TOTAL_SIZE_BYTES = 20 * 1024 * 1024; // 20 Mo
...
async function resolveCloudflareEnv(): Promise<{
  accountId: string;
  apiToken: string;
  imagesHash: string;
}> {
...
async function uploadToCloudflareWithCustomId(
  base64Data: string,
  mimeType: string,
  customId: string,
  accountId: string,
...
async function deleteFromCloudflare(
  imageId: string,
  accountId: string,
  apiToken: string
): Promise<void> {
...
> {
  ...
  const hashSuffix = shortHash(`${promptUsed}${Date.now()}`);
  const dateStr = yyyymmdd();
  const customId = `verone/marketing/${brandSlug}/${targetChannel}/${dateStr}-${presetId}-${hashSuffix}`;
