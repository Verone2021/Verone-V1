...
import { checkAndCleanExistingQuotes } from './_lib/duplicate-guard';
...
export async function fetchAndBuildOrderContext(
  ...
  // Guard anti-doublon : auto-overwrite si draft, 409 si finalisé/accepté (R4)
  const { conflict } = await checkAndCleanExistingQuotes(
    supabase,
    qontoClient,
    salesOrderId
  );
  if (conflict) {
    return {
      errorResponse: conflict as NextResponse<{
        success: boolean;
        error?: string;
      }>,
    };
