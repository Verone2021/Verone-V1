...
function buildApprovalBodyHtml(params: {
  orderNumber: string;
  organisationName: string | null;
  formattedTotal: string;
}): string {
  const { orderNumber, organisationName, formattedTotal } = params;
  const restaurantRow = organisationName
    ? `<tr>
...
function buildApprovalEmailHtml(params: {
  ...
  return buildEmailHtml({
    ...
    accentColor: 'green',
    bodyHtml: buildApprovalBodyHtml({
      orderNumber: params.orderNumber,
      organisationName: params.organisationName,
      formattedTotal: params.formattedTotal,
    }),
    ctaUrl: params.step4Url,
