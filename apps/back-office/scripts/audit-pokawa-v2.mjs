...
const validCommissions = (commissions || []).filter(c =>
  c.sales_orders?.status && !['cancelled', 'draft'].includes(c.sales_orders.status)
)

const totalAffiliateHT = validCommissions.reduce((s, c) => s + parseFloat(c.affiliate_commission || 0), 0)
const totalAffiliateTTC = validCommissions.reduce((s, c) => s + parseFloat(c.affiliate_commission_ttc || 0), 0)
const commissionsZero = validCommissions.filter(c => parseFloat(c.affiliate_commission || 0) === 0)
...
if (commissionsZero.length > 0) {
  ...
  commissionsZero.forEach(c => {
    ...
    console.log(`   ${order.order_number} | ${order.created_at?.substring(0,10)} | ${order.total_ht}€ HT`)
  })
  console.log()
}
...
if (problematic.length > 0) {
  ...
  const { data: items } = await supabase
    ...
    .select('order_id, product_name, quantity, total_ht, retrocession_amount, affiliate_payout_ht')
    .in('order_id', orderIds)

  const totalPayout = items.reduce((s, i) => s + parseFloat(i.affiliate_payout_ht || 0), 0)
  const commissionTheorique = totalPayout * 0.15

  console.log(`   Total affiliate_payout_ht: ${totalPayout.toFixed(2)} €`)
  console.log(`   Commission théorique (15%): ${commissionTheorique.toFixed(2)} €`)
