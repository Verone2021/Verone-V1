...
const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://aorroydfjsrygmosnzrl.supabase.co'
const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFvcnJveWRmanNyeWdtb3NuenJsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTc3Mjc0NzAsImV4cCI6MjA3MzMwMzQ3MH0.fRi7svC8bAr3wihH76pfQnaq7tjuBMypAEi6q-rHCTA'
...
const totalTTC = orders.reduce((sum, o) => sum + parseFloat(o.total_ttc || 0), 0)
...
if (commissionsError) {
  ...
  const totalAffiliateHT = commissions.reduce((sum, c) => sum + parseFloat(c.affiliate_commission || 0), 0)
  const totalAffiliateTTC = commissions.reduce((sum, c) => sum + parseFloat(c.affiliate_commission_ttc || 0), 0)
  const totalVeroneHT = commissions.reduce((sum, c) => sum + parseFloat(c.verone_commission || 0), 0)
  const totalVeroneTTC = commissions.reduce((sum, c) => sum + parseFloat(c.verone_commission_ttc || 0), 0)
  const commissionsZero = commissions.filter(c => parseFloat(c.affiliate_commission || 0) === 0).length
  const commissionsNeg = commissions.filter(c => parseFloat(c.affiliate_commission || 0) < 0).length
...
const { data: zeroCommissions, error: zeroError } = await supabase
  .from('linkme_commissions')
  .select(`
    id,
    order_id,
...
const ordersWithoutCommission = orders.filter(o => !ordersWithCommission.has(o.id))

console.log(`   Commandes avec commission = 0€: ${zeroCommissions?.length || 0}`)
...
if (zeroCommissions?.length > 0) {
  ...
  zeroCommissions.forEach(c => {
    console.log(`   - ${c.sales_orders.order_number} (${c.sales_orders.created_at?.substring(0,10)}) - HT: ${c.sales_orders.total_ht}€`)
  })
}
...
if (ordersWithoutCommission.length > 0) {
  ...
  ordersWithoutCommission.forEach(o => {
    console.log(`   - ${o.order_number} (${o.created_at?.substring(0,10)}) - HT: ${o.total_ht}€`)
  })
}
...
const problematicOrders = [
  'LINK-230026', 'LINK-240006', 'LINK-240009', 'LINK-240010',
  'LINK-240011', 'LINK-240012', 'LINK-240013', 'LINK-240014',
  'LINK-240016', 'LINK-240017', 'LINK-240019', 'LINK-240022',
  'LINK-240025', 'LINK-240031', 'LINK-240033', 'LINK-240038',
  'LINK-240046', 'LINK-240060', 'LINK-240075'
]
...
if (!problematicError && problematicOrdersData) {
  console.log(`   Trouvées: ${problematicOrdersData.length}/${problematicOrders.length}`)

  // Vérifier items de ces commandes
  const problematicOrderIds = problematicOrdersData.map(o => o.id)
  const { data: items, error: itemsError } = await supabase
    .from('sales_order_items')
    .select('order_id, product_name, quantity, unit_price_ht, total_ht, retrocession_amount, custom_commission_rate, affiliate_payout_ht')
    .in('order_id', problematicOrderIds)

  if (items) {
    const totalRetrocession = items.reduce((sum, i) => sum + parseFloat(i.retrocession_amount || 0), 0)
    console.log(`   Total items: ${items.length}`)
    console.log(`   Total retrocession_amount (legacy): ${totalRetrocession.toFixed(2)} €`)

    // Items avec affiliate_payout_ht
    const itemsWithPayout = items.filter(i => i.affiliate_payout_ht && parseFloat(i.affiliate_payout_ht) > 0)
    console.log(`   Items avec affiliate_payout_ht: ${itemsWithPayout.length}`)

    if (itemsWithPayout.length > 0) {
      const totalPayout = itemsWithPayout.reduce((sum, i) => sum + parseFloat(i.affiliate_payout_ht || 0), 0)
      console.log(`   Total affiliate_payout_ht: ${totalPayout.toFixed(2)} €`)
      console.log(`   Commission théorique (15%): ${(totalPayout * 0.15).toFixed(2)} €`)
    }
  }
}
