import type { NextRequest } from 'next/server';
import { NextResponse } from 'next/server';

import { Resend } from 'resend';

import { buildVeroneEmailHtml } from '@/emails/verone-email-template';
import { getLogoAttachments } from '../_shared/email-logo';

function getResendClient(): Resend | null {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) return null;
  return new Resend(apiKey);
}

interface ShippingNotificationRequest {
  email: string;
  customerName: string;
  orderId: string;
  trackingNumber?: string;
  carrierName?: string;
  estimatedDelivery?: string;
}

export async function POST(request: NextRequest) {
  try {
    const body = (await request.json()) as ShippingNotificationRequest;

    if (!body.email || !body.orderId) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields' },
        { status: 400 }
      );
    }

    const resendClient = getResendClient();
    if (!resendClient) {
      return NextResponse.json({ success: true, emailDisabled: true });
    }

    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://verone.fr';
    const trackingHtml = body.trackingNumber
      ? `<p style="font-size: 14px; color: #374151; margin: 0 0 8px 0;"><strong>N&deg; de suivi :</strong> ${body.trackingNumber}${body.carrierName ? ` (${body.carrierName})` : ''}</p>`
      : '';

    const deliveryHtml = body.estimatedDelivery
      ? `<p style="font-size: 14px; color: #374151; margin: 0 0 8px 0;"><strong>Livraison estim&eacute;e :</strong> ${body.estimatedDelivery}</p>`
      : '';

    const emailHtml = buildVeroneEmailHtml({
      title: 'Votre commande est en route',
      recipientName: body.customerName,
      accentColor: 'blue',
      bodyHtml: `
        <p style="font-size: 15px; color: #374151; margin: 0 0 16px 0;">
          Votre commande <strong>#${body.orderId.slice(0, 8).toUpperCase()}</strong> a &eacute;t&eacute; exp&eacute;di&eacute;e.
        </p>
        <div style="background-color: #eff6ff; padding: 16px; margin: 0 0 16px 0; border-left: 3px solid #2563eb;">
          ${trackingHtml}
          ${deliveryHtml}
        </div>
        <p style="font-size: 14px; color: #6b7280; margin: 0;">
          Vous recevrez un email lorsque votre colis sera livr&eacute;.
        </p>
      `,
      ctaUrl: `${siteUrl}/compte`,
      ctaLabel: 'Suivre ma commande',
    });

    const result = await resendClient.emails.send({
      from: process.env.RESEND_FROM_EMAIL ?? 'V\u00e9rone <noreply@verone.fr>',
      to: body.email,
      subject: `Commande #${body.orderId.slice(0, 8).toUpperCase()} exp\u00e9di\u00e9e`,
      html: emailHtml,
      replyTo: process.env.RESEND_REPLY_TO ?? 'contact@veronecollections.fr',
      attachments: getLogoAttachments(),
    });

    return NextResponse.json({ success: true, emailId: result.data?.id });
  } catch (error) {
    console.error('[API Shipping Notification] error:', error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error',
      },
      { status: 500 }
    );
  }
}
