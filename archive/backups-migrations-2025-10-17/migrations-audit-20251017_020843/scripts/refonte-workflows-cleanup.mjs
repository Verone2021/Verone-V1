...
if (soError) {
  ...
  const prepayment = salesOrders.filter(so => so.payment_required === true).length
...
if (prodError) {
  ...
  const lowStock = products.filter(p => p.stock_real >= 1 && p.stock_real <= 10).length
...
const { error: mvtError } = await supabase
  ...
  .delete()
  .in('reference_type', ['sales_order', 'sales_order_forecast', 'purchase_order', 'purchase_order_forecast'])
...
if (poiError) {
  console.error('❌ Erreur suppression purchase_order_items:', poiError.message)
} else {
...
console.log(`  ✅ ${organisationsEncours.length} clients ENCOURS créés/mis à jour`)
console.log(`  ✅ ${organisationsPrepayment.length} clients PRÉPAIEMENT créés/mis à jour`)
console.log('')
