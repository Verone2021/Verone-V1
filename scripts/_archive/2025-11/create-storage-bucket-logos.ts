...
async function createStorageBucket() {
  ...
  try {
    ...
    const { data: bucketData, error: bucketError } =
      await supabase.storage.createBucket('organisation-logos', {
        public: true,
    ...
    console.log(
      '✅ Setup Storage bucket "organisation-logos" terminé avec succès!'
    );
    console.log('\n📝 Prochaines étapes:');
    console.log(
      '   1. Les RLS policies Storage sont déjà créées via migration SQL'
    );
    console.log('   2. La column logo_url existe dans la table organisations');
    console.log(
      '   3. Les composants OrganisationLogo et LogoUploadButton sont prêts'
    );
