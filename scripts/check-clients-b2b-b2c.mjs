...
if (indError) {
  ...
  if (individuals.length > 0) {
    ...
    individuals.forEach(ind => {
      console.log(
        `  - ${ind.first_name} ${ind.last_name} (${ind.email || "pas d'email"})`
      );
    });
  } else {
...
if (jeanMartin) {
  console.log(
    '🔴 PROBLÈME: "Jean Martin" existe comme ORGANISATION (table organisations)'
  );
...
if (marieDupont) {
  console.log(
    '🔴 PROBLÈME: "Marie Dupont" existe comme ORGANISATION (table organisations)'
  );
...
if (jeanMartin || marieDupont) {
  ...
  console.log(
    '   → Organisations = Entreprises B2B (Pokawa, Hotel Le Luxe, etc.)'
  );
  console.log(
    '   → Individual_customers = Clients particuliers B2C (Jean Martin, Marie Dupont, etc.)'
  );
  console.log('');
  console.log('📝 RECOMMANDATION:');
  console.log(
    '   1. Supprimer "Jean Martin" et "Marie Dupont" de la table organisations'
  );
  console.log('   2. OU les migrer vers individual_customers');
  console.log(
    '   3. OU les renommer en noms d\'entreprise (ex: "Jean Martin SARL")'
  );
} else {
  console.log(
    '✅ Pas de pollution : organisations contient uniquement des entreprises'
  );
}
