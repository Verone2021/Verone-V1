...
const COMPONENTS_BUSINESS_DIR = path.join(PROJECT_ROOT, 'src/components/business');
...
function scanDirectory(dir) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      const businessMatches = [...content.matchAll(/@\/components\/business\/([a-z0-9-]+)/g)];
...
for (const imp of allImports) {
  const targetPath = imp.type === 'component'
...
if (generated.length > 0) {
  ...
  generated.forEach(g => {
    console.log(`   - ${g.type === 'component' ? 'src/components/business/' : 'src/hooks/'}${g.name}.${g.type === 'component' ? 'tsx' : 'ts'}`);
  });
}
...
console.log('\n✨ Script terminé !');
console.log('💡 Redémarrer le serveur Next.js pour appliquer les changements.\n');
