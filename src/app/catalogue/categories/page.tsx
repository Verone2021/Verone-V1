...
export default function CategoriesPage() {
  ...
  const getCategoriesForFamily = (familyId: string): CategoryWithChildren[] => {
    ...
    return allCategories.filter(cat => cat.family_id === familyId).map(cat => ({ ...cat, children: [], level: cat.level || 0 }))
  }

  // 🔍 FONCTION POUR OBTENIR LES SOUS-CATÉGORIES D'UNE CATÉGORIE
  const getSubcategoriesForCategory = (categoryId: string): SubcategoryWithDetails[] => {
  ...
  const openCreateForm = (type: 'family' | 'category' | 'subcategory', parentId?: string) => {
    setFormState({
      isOpen: true,
      type,
      mode: 'create',
  ...
  const openEditForm = (type: 'family' | 'category' | 'subcategory', data: any) => {
    setFormState({
      isOpen: true,
      type,
      mode: 'edit',
      ...
      if (formState.mode === 'create') {
        switch (formState.type) {
          case 'family':
            ...
            break
          case 'category':
            await createCategory({ ...formData, family_id: formState.parentId })
            break
          case 'subcategory':
            await createSubcategory({ ...formData, category_id: formState.parentId })
            break
        }
      } else {
        switch (formState.type) {
  ...
  const handleDelete = async (type: 'family' | 'category' | 'subcategory', id: string) => {
    ...
    try {
      ...
      alert("Erreur lors de la suppression. Vérifiez qu'il n'y a pas d'éléments liés.")
    }
  }
  ...
  const FamilyRow = ({ family }: { family: FamilyWithStats }) => {
    ...
    return (
      <div className="border-b border-gray-100">
        ...
        {isExpanded && familyCategories.map(category => (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Catalogue - Hiérarchie</h1>
      ...
      {hasError && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          ...
          {familiesError && <p className="text-red-700 text-sm mt-1">Familles: {familiesError}</p>}
          {categoriesError && <p className="text-red-700 text-sm mt-1">Catégories: {categoriesError}</p>}
          {subcategoriesError && <p className="text-red-700 text-sm mt-1">Sous-catégories: {subcategoriesError}</p>}
        </div>
      )}
