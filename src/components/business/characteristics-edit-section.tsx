...
import { RoomType, getRoomLabel } from '../../types/room-types'
...
const formatDimension = (value: string): string => {
  ...
  if (value.includes('cm') || value.includes('mm') || value.includes('m') || value.includes('kg') || value.includes('g')) {
    return value
  }
...
export function CharacteristicsEditSection({
      ...
      <div className={cn("bg-white border border-black p-4", className)}>
        ...
        <div className="space-y-4">
          ...
          {Object.keys(editData?.variant_attributes || {}).length > 0 && (
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(editData?.variant_attributes || {}).map(([key, value]) => (
          ...
          <div className="space-y-4">
            ...
            {Object.keys(editData?.dimensions || {}).length > 0 && (
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries(editData?.dimensions || {}).map(([key, value]) => (
                    <Badge key={key} variant="secondary" className="flex items-center gap-1">
                      <span className="font-medium">{key}:</span>
                      <span>{value as string}</span>
                      <Button
              ...
              <div className="flex gap-2">
                ...
                <div className="flex-1 relative">
                  <Input
                    ...
                    placeholder={newDimensionKey.toLowerCase().includes('poids') ? "2.5" : "120"}
            ...
            <div className="space-y-2">
              <Label className="text-sm text-gray-600">Dimensions suggérées :</Label>
              <div className="flex flex-wrap gap-2">
                {[
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Sélectionner les pièces où ce produit peut être utilisé :</Label>
              <RoomMultiSelect
              ...
              />
              <div className="text-xs text-gray-500">
                Cette information aide les clients à trouver le produit selon leur besoin par pièce.
              </div>
            </div>
          </div>
        </div>
  ...
  const hasCharacteristics = Object.values(characteristics).some(value => value && value.toString().trim() !== '')
  const hasDimensions = Object.values(dimensions).some(value => value && value.toString().trim() !== '')
      ...
      {hasAnyData ? (
                ...
                {Object.entries(characteristics).map(([key, value]) => (
                  value && (
                    <div key={key} className="flex justify-between items-start">
                      <div className="text-sm font-medium text-gray-600 min-w-[120px]">{key}:</div>
                      <div className="text-gray-900 text-sm text-right flex-1">{value as string}</div>
                    </div>
                  )
                ))}
              </div>
              ...
              <div className="space-y-3">
                {Object.entries(dimensions).map(([key, value]) => (
                  value && (
                    <div key={key} className="flex justify-between items-start">
                      <div className="text-sm font-medium text-gray-600 min-w-[120px]">{key}:</div>
                      <div className="text-gray-900 text-sm text-right flex-1">{formatDimension(value as string)}</div>
                    </div>
                  )
                ))}
              </div>
          ...
          {hasRooms && (
            <div className="space-y-3">
              ...
              <div className="text-xs text-gray-500 mt-2">
                Adapté pour {rooms.length} pièce{rooms.length > 1 ? 's' : ''} différente{rooms.length > 1 ? 's' : ''}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="text-center text-gray-400 text-sm italic py-6">
          <div className="space-y-2">
            ...
            <div className="text-xs">Cliquez sur "Modifier" pour ajouter des informations</div>
          </div>
        </div>
      )}
    </div>
