...
export function MovementsTable({ movements, loading, onMovementClick, onCancelClick, onOrderClick }: MovementsTableProps) {
  ...
  const getMovementTypeBadge = (movementType: string) => {
    switch (movementType) {
      case 'IN':
        return <Badge variant="secondary" className="bg-green-100 text-green-800 hover:bg-green-100">Entrée</Badge>
      case 'OUT':
        return <Badge variant="destructive">Sortie</Badge>
      case 'ADJUST':
        return <Badge variant="secondary" className="bg-blue-100 text-blue-800 hover:bg-blue-100">Ajustement</Badge>
      case 'TRANSFER':
        return <Badge variant="secondary" className="bg-purple-100 text-purple-800 hover:bg-purple-100">Transfert</Badge>
      default:
