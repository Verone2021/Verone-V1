...
const COLLECTION_STYLES: { value: CollectionStyle; label: string; description: string }[] = [
  ...
  { value: 'contemporain', label: 'Contemporain', description: 'Moderne et actuel' },
  { value: 'moderne', label: 'Moderne', description: 'Design avant-gardiste' },
  { value: 'scandinave', label: 'Scandinave', description: 'Chaleureux et lumineux' },
  { value: 'industriel', label: 'Industriel', description: 'Brut et authentique' },
  { value: 'classique', label: 'Classique', description: 'Intemporel et élégant' },
  { value: 'boheme', label: 'Bohème', description: 'Libre et éclectique' },
  { value: 'art_deco', label: 'Art Déco', description: 'Raffiné et géométrique' },
]
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Catégorie de pièce */}
          <div className="space-y-3">
            <Label className="text-sm font-medium">Destination / Pièce</Label>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
              {ROOM_CATEGORIES.map((room) => (
                <button
                  key={room.value}
                  type="button"
                  onClick={() => setRoomCategory(roomCategory === room.value ? undefined : room.value)}
                  className={cn(
                    "p-3 rounded-lg border text-center transition-all",
                    roomCategory === room.value
                      ? "border-black bg-black text-white"
                      : "border-gray-300 hover:border-gray-400"
                  )}
                >
