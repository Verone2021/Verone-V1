...
export function SmartSuggestionsPanel({
      ...
      <div className={cn("bg-gray-50 rounded-lg p-4", className)}>
        {showHeader && (
          <div className="flex items-center space-x-2 mb-3">
            ...
            <span className="text-sm font-medium text-gray-600">Suggestions intelligentes</span>
          </div>
        )}
        <div className="text-center py-6">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Remplissez plus d'informations pour obtenir des suggestions personnalisées
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className={cn("bg-gradient-to-br from-blue-50 to-purple-50 rounded-lg p-4 border border-blue-100", className)}>
      {showHeader && (
        <div className="flex items-center space-x-2 mb-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-gray-800">Suggestions IA</span>
          </div>
            ...
            <div className="space-y-2">
              {typeSuggestions.slice(0, 3).map((suggestion, index) => {
                ...
                const isAlreadySelected =
                  (suggestion.type === 'style' && context.style === suggestion.value) ||
                  (suggestion.type === 'room' && context.room_category === suggestion.value) ||
                  (suggestion.type === 'tag' && context.existing_tags?.includes(suggestion.value))
                ...
                return (
                  <div
                    ...
                    onClick={!isAlreadySelected && !isApplied ? () => handleApplySuggestion(suggestion) : undefined}
                  >
      ...
      {analytics && (
        <div className="mt-4 pt-3 border-t border-blue-200">
          <div className="flex items-center space-x-2 text-xs text-gray-600">
            ...
            <span>
              Basé sur l'analyse de {Object.values(analytics.style_popularity).reduce((a, b) => a + b, 0)} collections existantes
            </span>
          </div>
        </div>
      )}
