...
export function ProductConsultationManager({
  ...
  const {
    ...
    assignProduct,
    removeProduct
  } = useConsultationProducts(consultationId)
  ...
  const handleRemoveLink = async (consultationProductId: string) => {
    if (window.confirm('Êtes-vous sûr de vouloir retirer ce produit de la consultation ?')) {
  ...
  return (
    <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {consultations.length > 0
                    ? Math.round((consultations.filter(c => c.status === 'terminee').length / consultations.length) * 100)
                    : 0}%
                </p>
              </div>
                  ...
                  {filteredConsultations.map(consultation => (
                    <SelectItem key={consultation.id} value={consultation.id}>
                      <div className="flex flex-col">
                        <span className="font-medium">{consultation.organisation_name}</span>
      ...
      <Alert>
        ...
        <AlertDescription>
          <div className="space-y-2">
            <div className="font-medium">Workflow associations produits-consultations :</div>
            <div className="text-sm space-y-1">
              <p>• <strong>Seuls les produits sourcing</strong> peuvent être associés aux consultations</p>
              <p>• <strong>Une proposition principale</strong> par consultation maximum</p>
              <p>• <strong>Prix personnalisés</strong> pour chaque association</p>
              <p>• <strong>Traçabilité complète</strong> des propositions commerciales</p>
            </div>
          </div>
        </AlertDescription>
      </Alert>
