  ...
  function StockEditSection({
    ...
    const getStockStatus = (quantity: number, minLevel: number) => {
      ...
      if (quantity <= minLevel)
        return { color: 'text-black', level: 'Critique' };
      if (quantity <= minLevel * 2)
        return { color: 'text-gray-700', level: 'Faible' };
      return { color: 'text-green-600', level: 'Bon' };
    };

    // ✅ BUSINESS RULE: Précommande/Arrêté → min_stock MUST be 0
    const isMinStockDisabled =
      editData?.product_status === 'preorder' ||
      editData?.product_status === 'discontinued';

    // ✅ Forcer min_stock=0 automatiquement si statut change vers preorder/discontinued
    React.useEffect(() => {
      if (isMinStockDisabled && editData?.min_stock !== 0) {
        handleFieldChange('min_stock', 0);
      }
    }, [editData?.product_status, isMinStockDisabled]);
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="bg-gray-100 p-3 rounded-md border-2 border-gray-200">
                <div className="flex items-center justify-between">
                  <div>
                    <div
                      className={cn('text-2xl font-bold', stockStatus.color)}
                    >
                  ...
                  <div className="text-right">
                    <div
                      className={cn('text-sm font-medium', stockStatus.color)}
                    >
                      {stockStatus.level}
                    </div>
            ...
            <div>
              ...
              <input
                ...
                min="0"
                step="1"
                disabled={isMinStockDisabled}
              />
              {isMinStockDisabled ? (
                <div className="text-xs text-blue-600 mt-1 flex items-center">
                  <AlertTriangle className="h-3 w-3 mr-1" />
                  ⚠️ Seuil forcé à 0 pour statuts Précommande/Arrêté (alertes
                  supprimées)
                </div>
              ) : (
            ...
            {editData &&
              editData.stock_quantity <= (editData.min_stock || 5) && (
