...
interface EditSourcingProductModalProps {
  ...
  onUpdate: (productId: string, data: {
    name?: string
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl text-black">Modifier le produit sourcing</DialogTitle>
          <DialogDescription>
            Modifiez les informations du produit en sourcing. Les champs marqués d'une * sont obligatoires.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (errors.margin_percentage) setErrors(prev => ({ ...prev, margin_percentage: '' }))
                }}
                placeholder="50"
                className={cn(
                  "transition-colors",
                  errors.margin_percentage && "border-red-300 focus:border-red-500"
                )}
              />
            </div>
            ...
            {formData.cost_price > 0 && formData.margin_percentage > 0 && (
              <p className="text-xs text-gray-500">
                Prix de vente calculé: {new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-gray-500">
              Assignez un fournisseur pour activer les liens fournisseur et permettre la validation
            </p>
          </div>
