...
export function useStockInventory() {
  ...
  const fetchInventory = useCallback(async (filters?: {
    ...
    try {
      ...
      const inventoryPromises = products.map(async (product) => {
        ...
        // Filtres de période
        if (filters?.dateFrom) {
          movementsQuery = movementsQuery.gte('performed_at', filters.dateFrom)
        }
        if (filters?.dateTo) {
        ...
        const adjustMovements = movements?.filter(m => m.movement_type === 'ADJUST') || []

        // Calculer totaux par type
        const total_in = inMovements.reduce((sum, m) => sum + m.quantity_change, 0)
        const total_out = outMovements.reduce((sum, m) => sum + Math.abs(m.quantity_change), 0)
        const total_adjustments = adjustMovements.reduce((sum, m) => sum + m.quantity_change, 0)

        const last_movement_at = movements && movements.length > 0
      ...
      const errorMessage = err.message || 'Erreur lors du chargement de l\'inventaire'
  ...
  const exportInventoryCSV = useCallback((data: ProductInventory[]) => {
    const csvRows = [
      ['Produit', 'SKU', 'Entrées', 'Sorties', 'Ajustements', 'Stock Actuel', 'Dernière Activité'].join(','),
      ...data.map(item => [
        `"${item.name}"`,
        item.sku,
        item.total_in,
        ...
        item.stock_quantity,
        item.last_movement_at ? new Date(item.last_movement_at).toLocaleDateString('fr-FR') : 'N/A'
      ].join(','))
    ]
    ...
    link.setAttribute('download', `inventaire-stock-${new Date().toISOString().split('T')[0]}.csv`)
