...
export const AGING_BUCKETS = [
  ...
  { id: '61-90', label: '61-90 jours', min: 61, max: 90, color: 'bg-yellow-100' },
  { id: '91-180', label: '91-180 jours', min: 91, max: 180, color: 'bg-orange-100' },
  { id: '180+', label: '180+ jours', min: 181, max: Infinity, color: 'bg-red-100' }
] as const
...
export function useAgingReport() {
  ...
  const generateReport = useCallback(async (dateFrom?: string, dateTo?: string) => {
    setLoading(true)
    setError(null)
    ...
    try {
      ...
      const bucketStats = AGING_BUCKETS.map(bucket => {
        const productsInBucket = productsWithAge.filter(p => p.bucket === bucket.id)

        return {
          bucket_id: bucket.id,
          label: bucket.label,
          count: productsInBucket.length,
          quantity: productsInBucket.reduce((sum, p) => sum + p.stock_quantity, 0),
      ...
      bucketStats.forEach(bucket => {
        bucket.percentage = totalValue > 0 ? (bucket.value / totalValue) * 100 : 0
      })
      ...
      const totalQuantity = productsWithAge.reduce((sum, p) => sum + p.stock_quantity, 0)
      const averageAge = totalProducts > 0
        ? productsWithAge.reduce((sum, p) => sum + p.age_days, 0) / totalProducts
        : 0

      // Produits > 90 jours
      const productsOver90Days = productsWithAge.filter(p => p.age_days > 90)
      const percentOver90Days = totalProducts > 0
        ? (productsOver90Days.length / totalProducts) * 100
        : 0

      // Valeur immobilisée (stock > 90 jours)
      const immobilizedValue = productsOver90Days.reduce((sum, p) => sum + p.value, 0)
      ...
      const errorMessage = err.message || 'Erreur lors de la génération du rapport aging'
