  ...
  useEffect(() => {
    async function fetchAnalytics() {
      try {
        ...
        const revenueByDay = (salesOrders || []).reduce((acc: Record<string, number>, order) => {
        ...
        const productsData: ProductsDataPoint[] = Object.entries(productsByWeek)
          .map(([week, count]) => ({
        ...
        const movementsByDay = (movements || []).reduce((acc: Record<string, { entrees: number, sorties: number }>, mov) => {
        ...
        const stockMovementsData: StockMovementDataPoint[] = Object.entries(movementsByDay)
        ...
        // Grouper par semaine
        const purchaseByWeek = (purchaseOrders || []).reduce((acc: Record<string, number>, order) => {
        ...
        const purchaseOrdersData: PurchaseOrderDataPoint[] = Object.entries(purchaseByWeek)
          .map(([week, amount]) => ({
            week: `S${Math.ceil((new Date(week).getDate()) / 7)}`,
