...
export function useCustomers(filters?: CustomerFilters) {
  ...
  const fetchCustomers = async () => {
    ...
    try {
      ...
      if (!customerType || customerType === 'all' || customerType === 'professional') {
        ...
        console.log(`🏢 Organisations chargées: ${professionalCustomers.length}`)
      }

      // Récupérer les clients particuliers (individual_customers)
      if (!customerType || customerType === 'all' || customerType === 'individual') {
        ...
        if (filters?.search) {
          individualQuery = individualQuery.or(`first_name.ilike.%${filters.search}%,last_name.ilike.%${filters.search}%,email.ilike.%${filters.search}%`)
        }
        ...
        const individualCustomers: UnifiedCustomer[] = (individualData || []).map(individual => ({
          id: individual.id,
          type: 'individual' as CustomerType,
          displayName: `${individual.first_name || ''} ${individual.last_name || ''}`.trim() || 'Client particulier',
      ...
      console.log(`✅ Total clients chargés: ${allCustomers.length} (type: ${customerType || 'all'})`)
  ...
  useEffect(() => {
    console.log(`🔄 useCustomers: customerType=${filters?.customerType}, search=${filters?.search}`)
    fetchCustomers()
  }, [filters?.customerType, filters?.search, filters?.is_active])

  const getCustomerById = async (id: string, type: CustomerType): Promise<UnifiedCustomer | null> => {
    try {
