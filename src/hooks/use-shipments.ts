...
export function useShipments() {
   ...
   */
  const createPacklinkShipment = useCallback(async (request: CreateShipmentRequest) => {
    setLoading(true)
    try {
   ...
   */
  const createChronoTrackShipment = useCallback(async (request: CreateShipmentRequest) => {
    setLoading(true)
    try {
   ...
   */
  const createManualShipment = useCallback(async (request: CreateShipmentRequest) => {
    setLoading(true)
    try {
      // 1. Créer shipment dans Supabase
      const { data: shipment, error: shipmentError } = await supabase
   ...
   */
  const fetchShipmentsForOrder = useCallback(async (salesOrderId: string) => {
    setLoading(true)
    try {
