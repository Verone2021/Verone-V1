...
export interface UseStockCoreReturn {
  ...
  getStockItems: (filters?: { search?: string; archived?: boolean }) => Promise<StockItem[]>
  ...
  getMovementsByChannel: (channelId: string, dateFrom?: string, dateTo?: string) => Promise<StockMovement[]>
...
export function useStockCore({
   ...
   */
  const getStockItems = useCallback(async (filters?: {
    search?: string
    archived?: boolean
  }): Promise<StockItem[]> => {
    setLoading(true)
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Erreur fetch stock items'
      setError(errorMsg)
      throw err
    } finally {
      setLoading(false)
   ...
   */
  const getStockItem = useCallback(async (productId: string): Promise<StockItem | null> => {
    try {
   ...
   */
  const getMovements = useCallback(async (filters?: MovementFilters): Promise<StockMovement[]> => {
    setLoading(true)
    setError(null)
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Erreur fetch movements'
      setError(errorMsg)
      throw err
    } finally {
      setLoading(false)
   ...
   */
  const createMovement = useCallback(async (params: CreateMovementParams): Promise<StockMovement> => {
    try {
      ...
      const quantityBefore = params.affects_forecast
        ? (params.forecast_type === 'out' ? stockBefore.stock_forecasted_out : stockBefore.stock_forecasted_in)
      ...
      if (
        ...
        console.log(`✅ [useStockCore] Auto-injection channel_id: ${channelId} (OUT sale)`)
      } else {
        console.log(`ℹ️ [useStockCore] Pas de channel_id (type=${params.movement_type}, ref=${params.reference_type})`)
      }
      ...
      console.log(`✅ [useStockCore] Mouvement créé: ${data.id} (${params.movement_type}, channel=${finalChannelId || 'NULL'})`)
   ...
   */
  const filterByChannel = useCallback(async (targetChannelId: string): Promise<StockMovement[]> => {
   ...
   */
  const getMovementsByChannel = useCallback(async (
