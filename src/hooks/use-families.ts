...
export function useFamilies() {
  ...
  const createFamily = async (familyData: Omit<FamilyInsert, 'id' | 'created_at' | 'updated_at'>) => {
    try {
  ...
  const deleteFamily = async (id: string) => {
    try {
      ...
      if (categories && categories.length > 0) {
        throw new Error(`Impossible de supprimer cette famille car elle contient ${categories.length} catégorie(s). Supprimez d'abord les catégories.`)
      }
  ...
  const toggleFamilyStatus = async (id: string, isActive: boolean) => {
    try {
      ...
      console.log(`✅ Famille ${isActive ? 'activée' : 'désactivée'}:`, data.name)
