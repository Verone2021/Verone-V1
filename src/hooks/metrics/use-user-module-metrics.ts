 ...
 */
const MODULE_CONFIG: Record<string, { name: string, icon: string, color: string }> = {
 ...
 */
export function useUserModuleMetrics(userId: string, days: number = 30): UserModuleMetricsData {
  ...
  useEffect(() => {
    const fetchModuleMetrics = async () => {
      try {
        ...
        if (activitiesError) {
          throw new Error(`Erreur fetch activities: ${activitiesError.message}`)
        }
        ...
        const total_time_calc = Object.values(moduleStats).reduce((sum, stat) => sum + stat.time_spent, 0)
        const total_page_views_calc = Object.values(moduleStats).reduce((sum, stat) => sum + stat.page_views, 0)

        // 📊 TRANSFORMATION: Créer array de modules avec pourcentages
        const modulesArray: ModuleUsageMetric[] = Object.entries(moduleStats)
          .map(([module, stats]) => {
            const config = MODULE_CONFIG[module] || { name: module, icon: 'Circle', color: 'gray' }
            ...
            return {
              ...
              percentage: total_time_calc > 0 ? Math.round((stats.time_spent / total_time_calc) * 100) : 0,
 ...
 */
export function getProgressBarColor(percentage: number, baseColor: string): string {
