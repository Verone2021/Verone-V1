...
export function useVariantProducts() {
  ...
  const addProductToVariantGroup = async (productId: string, variantGroupId: string, position?: number): Promise<boolean> => {
    try {
      setLoading(true)
      setError(null)
      ...
      if (existingProducts && existingProducts.length >= 30) {
        toast({
          title: "Erreur",
          description: "Maximum 30 variantes par groupe (limite Google Merchant Center)",
          variant: "destructive"
        })
        return false
      }
  ...
  const removeProductFromVariantGroup = async (productId: string): Promise<boolean> => {
    try {
      setLoading(true)
      setError(null)
  ...
  const createVariantProduct = async (variantGroupId: string, baseProductId: string, data: CreateVariantProductData) => {
    try {
      setLoading(true)
      setError(null)
      ...
      const variantSuffix = Object.entries(data.variant_attributes)
        ...
        .map(([key, value]) => `${key.charAt(0).toUpperCase()}${value.toString().substring(0, 3)}`)
        .join('-')
  ...
  const createQuickVariantProduct = async (variantGroupId: string, baseProductId: string, groupName: string, data: QuickVariantProductData) => {
    ...
    return await createVariantProduct(variantGroupId, baseProductId, fullProductData)
  }

  // Mettre à jour l'ordre des produits dans un groupe
  const reorderProductsInGroup = async (variantGroupId: string, productPositions: { productId: string; position: number }[]): Promise<boolean> => {
    try {
      setLoading(true)
      setError(null)
  ...
  const setVariantParent = async (productId: string, variantGroupId: string): Promise<boolean> => {
    try {
      setLoading(true)
      setError(null)
  ...
  const getAvailableProductsForVariantGroup = useCallback(async (search?: string, limit?: number) => {
    try {
      let query = supabase
        .from('products')
