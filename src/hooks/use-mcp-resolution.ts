...
export type ErrorType = 'typescript' | 'supabase_rls' | 'ui_component' | 'network' | 'performance'
export type ResolutionStatus = 'pending' | 'processing' | 'completed' | 'failed' | 'retrying'
...
const MCP_ERROR_RESOLUTION = {
  typescript: ['serena_find_symbol', 'serena_replace_symbol', 'ide_diagnostics'],
  supabase_rls: ['supabase_execute_sql', 'supabase_get_advisors', 'supabase_get_logs'],
  ui_component: ['playwright_snapshot', 'serena_find_symbol', 'serena_replace_symbol'],
...
const ERROR_DETECTION_LAYERS = {
  console: async () => {
    ...
    try {
      const logs = await (window as any).__PLAYWRIGHT_CONSOLE_MESSAGES?.() || []
  ...
  network: async () => {
    ...
    try {
      const requests = await (window as any).__PLAYWRIGHT_NETWORK_REQUESTS?.() || []
  ...
  performance: async () => {
    ...
    const metrics = {
      ...
      domReady: entries ? entries.domContentLoadedEventEnd - entries.domContentLoadedEventStart : 0
    }
...
export function useMCPResolution() {
  ...
  const detectErrors = useCallback(async () => {
    ...
    try {
      ...
      const [consoleErrors, networkErrors, supabaseErrors, ideErrors, performanceErrors] = await Promise.all([
      ...
      const newErrors = classifiedErrors.filter(error =>
        !errors.find(e => e.message === error.message && e.type === error.type)
      )
  ...
  const classifyErrors = useCallback(async (rawErrors: any[]): Promise<VeroneError[]> => {
    return rawErrors.map((error, index) => {
      // Déterminer le type d'erreur
      let errorType: ErrorType = 'ui_component'
      if (error.source === 'network') errorType = 'network'
      else if (error.source === 'supabase' || error.message?.includes('RLS')) errorType = 'supabase_rls'
      else if (error.source === 'ide' || error.message?.includes('Type')) errorType = 'typescript'
      else if (error.source === 'performance') errorType = 'performance'

      // Déterminer la sévérité
      let severity: ErrorSeverity = 'medium'
      if (error.message?.includes('CRITICAL') || error.status === 500) severity = 'critical'
      else if (error.message?.includes('Error') || error.status >= 400) severity = 'high'
      ...
      return {
        ...
        fix_priority: severity === 'critical' ? 10 : severity === 'high' ? 7 : 5,
        estimated_fix_time: confidence > 0.8 ? '30s' : confidence > 0.5 ? '5min' : '15min',
  ...
  const calculateConfidence = (type: ErrorType, message: string): number => {
    ...
    const highConfidencePatterns = {
      typescript: [/Cannot find module/, /is not defined/, /Type.*does not exist/],
      supabase_rls: [/row-level security/, /permission denied/, /insufficient_privilege/],
      ui_component: [/Cannot read properties of undefined/, /is not a function/],
  ...
  const queueForResolution = useCallback(async (error: VeroneError) => {
  ...
  const processResolutionQueue = useCallback(async () => {
    ...
    const updatedTask = {
      ...task,
      status: result.success ? 'completed' : task.retry_count >= task.max_retries ? 'failed' : 'retrying',
    ...
    if (result.success) {
      setStats(prev => ({
        ...
        resolution_rate: Math.round(((prev.auto_resolved + 1) / (prev.total_errors || 1)) * 100)
      }))
    }
  ...
  const simulateMCPResolution = async (task: MCPResolutionTask): Promise<ResolutionResult> => {
    ...
    return {
      ...
      suggestions: success ? undefined : ['Manual intervention required', 'Check error logs']
    }
  }
  ...
  }, [detectErrors, processResolutionQueue, errors, stats, resolutionQueue, supabase])
