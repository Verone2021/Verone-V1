  ...
  const fetchDashboardMetrics = useCallback(async () => {
    ...
    try {
      ...
      const { data: consultations, error: consultationsError } = await supabase
        ...
        .select('id, organisation_name, client_email, status, created_at, tarif_maximum')
        .neq('status', 'closed')
      ...
      const { data: orders, error: ordersError } = await supabase
        .from('sales_orders')
        .select('id, order_number, customer_id, customer_type, status, total_ttc, created_at')
      ...
      const chiffreAffaireMois = (monthlyOrders || []).reduce((sum, order) => sum + (order.total_ttc || 0), 0)
      ...
      const tauxConversion = totalConsultations > 0
      ...
      const errorMessage = err.message || 'Erreur lors du chargement du dashboard ventes'
