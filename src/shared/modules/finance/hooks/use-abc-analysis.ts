...
export const ABC_CLASSES = [
  {
    ...
    threshold: 0.8, // 80% de la valeur cumulée
    color: 'bg-green-100',
...
export function useABCAnalysis() {
  ...
  const generateReport = useCallback(async () => {
    ...
    try {
      ...
      const productsWithABC: ProductWithABC[] = productsWithValue.map(
        (product, index) => {
      ...
      const classStats: ABCClassData[] = ABC_CLASSES.map(abcClass => {
        const productsInClass = productsWithABC.filter(
          p => p.abc_class === abcClass.id
        );

        const classValue = productsInClass.reduce((sum, p) => sum + p.value, 0);
        const classQuantity = productsInClass.reduce(
          (sum, p) => sum + p.stock_quantity,
          0
        );

        return {
          class_id: abcClass.id,
      ...
      const top20LowValue = [...productsWithABC].reverse().slice(0, 20);

      // ============================================
      // CALCUL DES MÉTRIQUES GLOBALES
      // ============================================
      const totalProducts = productsWithABC.length;
      const totalQuantity = productsWithABC.reduce(
        (sum, p) => sum + p.stock_quantity,
        0
      );
      ...
      const classAValue = productsByClass.A.reduce(
        (sum, p) => sum + p.value,
        0
      );
      const classBValue = productsByClass.B.reduce(
        (sum, p) => sum + p.value,
        0
      );
      const classCValue = productsByClass.C.reduce(
        (sum, p) => sum + p.value,
        0
      );
      ...
      const reportData: ABCReportData = {
        summary: {
          ...
          class_a_value_percentage:
            totalValue > 0 ? (classAValue / totalValue) * 100 : 0,
          class_b_value_percentage:
            totalValue > 0 ? (classBValue / totalValue) * 100 : 0,
          class_c_value_percentage:
            totalValue > 0 ? (classCValue / totalValue) * 100 : 0,
        },
        classes: classStats,
