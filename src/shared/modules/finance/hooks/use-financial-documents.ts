...
export function useFinancialDocuments(filters?: FinancialDocumentFilters) {
  ...
  const createCustomerInvoice = async (salesOrderId: string) => {
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Erreur création facture';
      toast.error(message);
      throw err;
    }
  };
  ...
  const createExpense = async (params: {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Erreur création dépense';
      toast.error(message);
      throw err;
    }
  };
  ...
  const getStats = () => {
    ...
    return {
      ...
      net_cash_flow:
        inbound.reduce((sum, d) => sum + d.amount_paid, 0) -
