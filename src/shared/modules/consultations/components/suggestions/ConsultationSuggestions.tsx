    ...
    try {
      ...
      const clientConsultations = consultations.filter(consultation => {
        ...
        if (clientId) {
          ...
          return consultation.status === 'en_attente' || consultation.status === 'en_cours'
        }
        return false
      })
      ...
      <CardContent className="space-y-4">
        {relevantConsultations.map((consultation) => {
          ...
          const daysSinceCreation = Math.floor(
            (new Date().getTime() - new Date(consultation.created_at).getTime()) / (1000 * 60 * 60 * 24)
          )
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <div className="flex items-center space-x-4 text-xs text-gray-500">
                    <div className="flex items-center">
                      ...
                      Il y a {daysSinceCreation} jour{daysSinceCreation > 1 ? 's' : ''}
                    </div>
                    {consultation.tarif_maximum && (
                      <div>
                        Budget max : {consultation.tarif_maximum}€
                      </div>
                    )}
                    {consultation.estimated_response_date && (
                      <div>
                        Réponse prévue : {new Date(consultation.estimated_response_date).toLocaleDateString('fr-FR')}
                      </div>
                    )}
                  </div>
                </div>
        ...
        <Alert>
          ...
          <AlertDescription>
            <div className="text-sm">
              <strong>Conseil :</strong> Associer ce produit à une consultation permet un meilleur suivi commercial
              et aide à personnaliser la proposition client.
            </div>
          </AlertDescription>
        </Alert>
      </CardContent>
