  ...
  const fetchTopProducts = async () => {
    try {
      ...
      const { data: salesData, error: salesError } = await supabase
        ...
        .in('sales_orders.status', ['draft', 'confirmed', 'partially_shipped', 'shipped', 'delivered'])
      ...
      const trendPromises = topProductIds.map(async (productId) => {
        ...
        const prevQuantity = prevSales?.reduce((sum: number, item: any) => sum + (item.quantity || 0), 0) || 0
        ...
        return Math.round(((currentQuantity - prevQuantity) / prevQuantity) * 100)
      })
