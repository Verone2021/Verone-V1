...
export function useOrderItems({
  ...
  const table = orderType === 'purchase' ? 'purchase_order_items' : 'sales_order_items'
  const fkColumn = orderType === 'purchase' ? 'purchase_order_id' : 'sales_order_id'
  ...
  const addItem = useCallback(async (data: CreateOrderItemData): Promise<OrderItem> => {
    if (!orderId) {
    ...
    try {
      ...
      if (orderType === 'sales') {
        itemData.tax_rate = data.tax_rate || 0.20  // TVA 20% par défaut
      }
      ...
      console.log(`✅ [useOrderItems] Item ajouté (${orderType}):`, newItem.id)
      ...
      console.error(`❌ [useOrderItems] Erreur ajout item ${orderType}:`, err)
      throw err
    }
  ...
  const updateItem = useCallback(async (
    ...
    try {
      ...
      if (data.unit_price_ht !== undefined) updateData.unit_price_ht = data.unit_price_ht
      ...
      console.error(`❌ [useOrderItems] Erreur modification item ${orderType}:`, err)
      throw err
    }
  ...
  const removeItem = useCallback(async (itemId: string): Promise<void> => {
    try {
      ...
      console.error(`❌ [useOrderItems] Erreur suppression item ${orderType}:`, err)
      throw err
    }
  }, [orderType, table, supabase, fetchItems])
