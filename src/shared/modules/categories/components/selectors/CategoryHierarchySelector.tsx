...
interface CategoryHierarchySelectorProps {
  ...
  onChange: (subcategoryId: string | null, hierarchyInfo?: {
    family_id: string
    category_id: string
  ...
  useEffect(() => {
    if (value && subcategories.length > 0) {
      ...
      if (foundSubcategory && foundSubcategory.category) {
        const foundCategory = allCategories.find(cat => cat.id === foundSubcategory.category_id)
        if (foundCategory) {
          ...
          setCurrentHierarchy({
            ...
            family_name: families.find(f => f.id === foundCategory.family_id)?.name || '',
    ...
    <div className={cn("relative", className)}>
      ...
      <div
        className={cn(
          ...
          isOpen ? "border-black ring-2 ring-black ring-opacity-20" : "border-gray-300",
          ...
          {currentHierarchy ? (
            <div className="text-sm">
              <span className="text-black font-medium">
                {currentHierarchy.family_name} › {currentHierarchy.category_name} › {currentHierarchy.subcategory_name}
              </span>
            </div>
          ) : (
      ...
      {isOpen && !disabled && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-black rounded-md shadow-lg max-h-96 overflow-hidden">
          <div className="grid grid-cols-3 divide-x divide-gray-200 h-80">
                ...
                {families.map((family) => (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedFamily === family.id ? "bg-black text-white" : "text-gray-900"
                    )}
                  >
                    {family.name}
                  </div>
            ...
            <div className="overflow-y-auto">
              <div className="p-2 bg-gray-50 border-b">
                <h4 className="text-xs font-medium text-gray-700">Catégories</h4>
              </div>
              <div className="p-1">
                {selectedFamily ? (
                  filteredCategories.map((category) => (
                    <div
                      ...
                      className={cn(
                        "px-2 py-1 text-sm cursor-pointer rounded hover:bg-gray-100",
                        selectedCategory === category.id ? "bg-black text-white" : "text-gray-900"
                      )}
                    >
                      {category.name}
                    </div>
            ...
            <div className="overflow-y-auto">
              <div className="p-2 bg-gray-50 border-b">
                <h4 className="text-xs font-medium text-gray-700">Sous-catégories</h4>
              </div>
              <div className="p-1">
                {selectedCategory ? (
                  filteredSubcategories.map((subcategory) => (
                    <div
                      ...
                      className={cn(
                        ...
                        selectedSubcategory === subcategory.id ? "bg-green-600 text-white" : "text-gray-900"
                      )}
                    >
          ...
          <div className="p-2 bg-gray-50 border-t flex justify-between items-center">
            <div className="text-xs text-gray-500">
              {currentHierarchy ? "Sélection confirmée" : "Sélectionnez une sous-catégorie pour valider"}
            </div>
            <button
