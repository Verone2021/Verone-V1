...
export function CategoryHierarchyFilter({
  ...
  const getSelectedCountForFamily = (familyId: string) => {
    ...
    familyCategories.forEach(cat => {
      ...
      count += catSubcategories.filter(sub =>
    ...
    <div className={cn('space-y-2', className)}>
              ...
              {isExpanded && (
                <div className="ml-4 space-y-1">
                  {familyCategories.map(category => {
                    ...
                    const categorySelectedCount = getSelectedCountForCategory(
                      category.id
                    );
      ...
      {selectedSubcategories.length > 0 && (
        <div className="text-xs text-gray-600 flex items-center justify-between">
          <span>
            {selectedSubcategories.length} sous-catégorie
            {selectedSubcategories.length > 1 ? 's' : ''} sélectionnée
