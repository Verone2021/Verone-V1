...
import type { VeroneError } from './use-mcp-resolution';
import { MCPResolutionTask } from './use-mcp-resolution';
...
const AUTOMATION_PRESETS = {
  ...
  critical_error_alert: {
    ...
    actions: [
      {
        type: 'notify_admin' as AutomationAction,
        parameters: { channel: 'email', urgent: true },
        retry_count: 3,
        timeout_ms: 5000,
      },
      {
        type: 'create_ticket' as AutomationAction,
        parameters: { priority: 'P1', assignee: 'tech_lead' },
        retry_count: 2,
        timeout_ms: 10000,
      },
    ],
  ...
  performance_optimization: {
    ...
    conditions: [
      { field: 'load_time', operator: 'greater_than' as const, value: 3000 },
    ],
    actions: [
      {
        type: 'optimize_cache' as AutomationAction,
        parameters: { module: 'all', ttl: 3600 },
        retry_count: 1,
        timeout_ms: 15000,
      },
      {
        type: 'log_metric' as AutomationAction,
        parameters: { metric: 'performance_degradation' },
        retry_count: 1,
        timeout_ms: 2000,
      },
    ],
  ...
  auto_fix_known_errors: {
    ...
    actions: [
      {
        type: 'auto_fix' as AutomationAction,
        parameters: { use_mcp: true, backup_first: true },
        retry_count: 2,
        timeout_ms: 30000,
      },
    ],
    ...
    conditions: [
      {
        ...
        value: ['stocks', 'commandes', 'catalogue'],
      },
    ],
    actions: [
      {
        type: 'backup_data' as AutomationAction,
        parameters: { incremental: true, compress: true },
        retry_count: 1,
        timeout_ms: 60000,
      },
    ],
...
const AUTOMATION_EXECUTORS = {
  notify_admin: async (params: any, context: any) => {
    ...
    return {
      success: true,
      message: `Admin notifié via ${params.channel}`,
  ...
  auto_fix: async (params: any, context: any) => {
    ...
    if (params.use_mcp && context.error?.mcp_tools_needed?.length > 0) {
      ...
      return {
        ...
        data: {
          mcp_tools_used: context.error.mcp_tools_needed,
          execution_time: '15s',
        },
      };
    }
  ...
  log_metric: async (params: any, context: any) => {
    ...
    return {
      success: true,
      message: 'Métrique enregistrée',
      data: { metric: params.metric, timestamp: new Date() },
    };
  },
...
export function useAutomationTriggers() {
  ...
  const initializeDefaultTriggers = useCallback(() => {
    const defaultTriggers: AutomationTrigger[] = Object.entries(
      AUTOMATION_PRESETS
    ).map(([key, preset]) => ({
    ...
    setStats(prev => ({
      ...
      active_triggers: defaultTriggers.filter(t => t.status === 'active')
        .length,
    }));

    console.log(
      `🤖 ${defaultTriggers.length} triggers d'automatisation initialisés`
    );
  }, []);

  // Évaluer si un événement matche les conditions d'un trigger
  const evaluateConditions = useCallback(
    (conditions: TriggerCondition[], eventData: any): boolean => {
  ...
  const executeActions = useCallback(
    async (actions: TriggerAction[], context: any): Promise<any[]> => {
      const results: any[] = [];
      ...
      for (const action of actions) {
        try {
          ...
          if (!executor) {
            throw new Error(
              `Exécuteur non trouvé pour l'action: ${action.type}`
            );
          }
  ...
  const triggerAutomations = useCallback(
    async (eventType: TriggerEvent, eventData: any) => {
      ...
      console.log(
        `🤖 ${matchingTriggers.length} automatisations déclenchées pour événement: ${eventType}`
      );
      ...
      for (const trigger of matchingTriggers) {
        ...
        try {
            ...
            prev.map(t =>
              t.id === trigger.id
                ? {
                    ...
                    success_rate:
                      Math.round(
                        ((t.success_rate * t.execution_count +
                          (success ? 100 : 0)) /
                          (t.execution_count + 1)) *
                          100
                      ) / 100,
          ...
          console.log(
            `✅ Automation ${trigger.name} exécutée en ${duration}ms - Success: ${success}`
          );
        } catch (error) {
  ...
  const onErrorResolved = useCallback(
    (error: VeroneError, resolution: any) => {
      triggerAutomations('error_resolved', { ...error, resolution });
    },
    [triggerAutomations]
  );

  const onPerformanceDegraded = useCallback(
    (metrics: any) => {
  ...
  const onModuleAccessed = useCallback(
    (module: string, context: any) => {
  ...
  const createTrigger = useCallback(
    async (triggerData: Partial<AutomationTrigger>) => {
  ...
  const updateTrigger = useCallback(
    (triggerId: string, updates: Partial<AutomationTrigger>) => {
      setTriggers(prev =>
  ...
  return {
    ...
    getTriggersByEvent: (eventType: TriggerEvent) =>
      triggers.filter(t => t.event_type === eventType),
    getRecentExecutions: (limit = 10) => executions.slice(0, limit),
    getExecutionsByTrigger: (triggerId: string) =>
      executions.filter(e => e.trigger_id === triggerId),
  };
}
