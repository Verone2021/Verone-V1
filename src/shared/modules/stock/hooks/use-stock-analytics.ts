...
export function useStockAnalytics() {
   ...
   */
  const calculateCoefficientOfVariation = (values: number[]): { cv: number; mean: number; stddev: number } => {
   ...
   */
  const generateReport = useCallback(async (periodDays = 90) => {
    setLoading(true)
    setError(null)
    ...
    try {
      ...
      const { data, error: rpcError } = await (supabase as any)
        .rpc('get_stock_analytics', { p_period_days: periodDays, p_organisation_id: null })
      ...
      // ============================================
      const productsWithValue = rawProducts.map(product => ({
        ...product,
        stock_value: product.stock_current * product.cost_price
      })).sort((a, b) => b.stock_value - a.stock_value)

      const totalValue = productsWithValue.reduce((sum, p) => sum + p.stock_value, 0)
      ...
      // ============================================
      const productsWithXYZ: ProductAnalytics[] = productsWithABC.map(product => {
        ...
        const { cv, mean, stddev } = calculateCoefficientOfVariation(monthlySales)
      ...
      const totalQuantity = productsWithXYZ.reduce((sum, p) => sum + p.stock_current, 0)
      ...
      const avgADU = productsWithXYZ.reduce((sum, p) => sum + p.adu, 0) / totalProducts
      const avgTurnover = productsWithXYZ.reduce((sum, p) => sum + p.turnover_rate, 0) / totalProducts
      const avgCoverage = productsWithXYZ.filter(p => p.coverage_days < 900).reduce((sum, p) => sum + p.coverage_days, 0) / totalProducts

      const belowMin = productsWithXYZ.filter(p => p.stock_current <= p.stock_minimum).length
      const inactive30d = productsWithXYZ.filter(p => p.days_inactive !== null && p.days_inactive > 30).length
      ...
      const abcClassStats: AnalyticsClassData[] = ABC_CLASSES.map(cls => {
        ...
        const classValue = products.reduce((sum, p) => sum + p.stock_value, 0)
        const classQuantity = products.reduce((sum, p) => sum + p.stock_current, 0)
      ...
      for (const abcCls of ['A', 'B', 'C']) {
        for (const xyzCls of ['X', 'Y', 'Z']) {
          ...
          combinedMatrix[key] = productsWithXYZ.filter(p => p.combined_class === key)
        }
      }
      ...
      const reportData: AnalyticsReport = {
        summary: {
          ...
          abc_a_value_percentage: totalValue > 0 ? (abcA.reduce((sum, p) => sum + p.stock_value, 0) / totalValue) * 100 : 0,
          abc_b_value_percentage: totalValue > 0 ? (abcB.reduce((sum, p) => sum + p.stock_value, 0) / totalValue) * 100 : 0,
          abc_c_value_percentage: totalValue > 0 ? (abcC.reduce((sum, p) => sum + p.stock_value, 0) / totalValue) * 100 : 0,
      ...
      const errorMessage = err.message || 'Erreur lors de la génération du rapport analytics'
