...
export function useStockReservations() {
          ...
          (acc, reservation) => {
            ...
            if (!reservation.released_at) {
              ...
              if (
                ...
                new Date(reservation.expires_at) <= in24Hours
              ) {
    ...
    async (data: CreateReservationData) => {
      ...
      try {
        ...
        if (availableStock < data.reserved_quantity) {
          throw new Error(
            `Stock insuffisant. Disponible: ${availableStock}, Demandé: ${data.reserved_quantity}`
          );
        }
  ...
  const reserveForProduction = useCallback(
    (
      productId: string,
      quantity: number,
