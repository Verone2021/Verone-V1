    ...
    try {
      ...
      const [
        ...
        supabase
          .from('stock_alerts_view')
          .select(
            'product_id, product_name, sku, stock_quantity, min_stock, alert_status, alert_priority'
          )
      ...
      const productsMap = new Map(
        productsWithLegacyFields.map(p => [p.id, { name: p.name, sku: p.sku }])
      );
