...
import {
  ...
  useSupabaseMutation,
  clearQueryCache,
} from '@/shared/modules/common/hooks/use-supabase-query';
...
export function useStockOptimized(filters: StockFilters = {}) {
  ...
  const stockSummaryQuery = useSupabaseQuery(
    ...
    async supabase => {
      ...
      const { data: summaryData, error } =
        await supabase.rpc('get_stock_summary');
  ...
  const lowStockQuery = useSupabaseQuery(
    ...
    (async (supabase: ReturnType<typeof createClient>) => {
      ...
      if (error) {
        ...
        console.warn(
          'Fonction get_low_stock_products non disponible, utilisation requête alternative'
        );
  ...
  const createMovementMutation = useSupabaseMutation<StockMovement>((async (
  ...
  const createMovement = useCallback(
    async (
      movementData: Parameters<typeof createMovementMutation.mutate>[0]
    ) => {
  ...
  const adjustStock = useCallback(
    async (params: Parameters<typeof adjustStockMutation.mutate>[0]) => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      movements_today: movements.filter(
        m => new Date(m.performed_at).toDateString() === today
      ).length,
      movements_week: movements.filter(m => new Date(m.performed_at) >= weekAgo)
        .length,
