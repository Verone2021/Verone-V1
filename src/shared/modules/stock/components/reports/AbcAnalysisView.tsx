...
export function ABCAnalysisView({}: ABCAnalysisViewProps) {
  ...
  const handleExportCSV = () => {
    ...
    try {
      ...
      const csvData = [
        ['SKU', 'Produit', 'Stock', 'Prix Coût', 'Valeur', 'Classe ABC', 'Rang', '% Cumulé'],
        ...report.products_by_class.A.map(p => [
          p.sku, p.name, p.stock_quantity, p.cost_price, p.value, 'A', p.rank, p.cumulative_percentage.toFixed(2)
        ]),
        ...report.products_by_class.B.map(p => [
          p.sku, p.name, p.stock_quantity, p.cost_price, p.value, 'B', p.rank, p.cumulative_percentage.toFixed(2)
        ]),
        ...report.products_by_class.C.map(p => [
          p.sku, p.name, p.stock_quantity, p.cost_price, p.value, 'C', p.rank, p.cumulative_percentage.toFixed(2)
        ])
      ]
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold">{report.summary.total_products}</p>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold">{report.summary.total_value.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} €</p>
            <p className="text-xs text-gray-600 mt-1">
        ...
        <Card className="bg-green-50 border-green-200">
          ...
          <CardContent>
            <p className="text-3xl font-bold text-green-800">{report.summary.class_a_count}</p>
        ...
        <Card className="bg-blue-50 border-blue-200">
          ...
          <CardContent>
            ...
            <p className="text-xs text-blue-700 mt-1">
              {(report.summary.class_b_value_percentage + report.summary.class_c_value_percentage).toFixed(1)}% de la valeur
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <div className="space-y-3">
            {report.classes.map((classData) => (
              <div key={classData.class_id} className={`${classData.color} border rounded-lg p-4`}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <Badge className={`${classData.textColor} text-lg font-bold px-3 py-1`}>
                      {classData.class_id}
                    </Badge>
                    <div>
                      <p className="font-bold text-sm">{classData.label}</p>
                      <p className="text-xs text-gray-600">{classData.description}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-4 gap-4 mt-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold">{classData.value.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} €</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-600">% Valeur</p>
                    <p className="text-xl font-bold">{classData.percentage.toFixed(1)}%</p>
                  </div>
                </div>
            ...
            <TableBody>
              {report.top_20_high_value.map((product) => (
                <TableRow key={product.id}>
                  <TableCell className="font-mono text-xs">{product.rank}</TableCell>
                  <TableCell>
                    <Badge className={ABC_CLASSES.find(c => c.id === product.abc_class)?.color}>
                      {product.abc_class}
                    </Badge>
                  </TableCell>
      ...
      <Card className="bg-blue-50 border-blue-200">
        ...
        <CardContent className="space-y-2 text-sm">
          <div className="flex items-start gap-2">
            <span className="font-bold text-green-800">Classe A ({report.summary.class_a_count} produits)</span>
            <span className="text-gray-700">
              → Monitoring quotidien, réapprovisionnement prioritaire, stock sécurité élevé
            </span>
          </div>
          <div className="flex items-start gap-2">
            <span className="font-bold text-blue-800">Classe B ({report.summary.class_b_count} produits)</span>
            <span className="text-gray-700">
              → Suivi hebdomadaire, réappro standard, stock sécurité modéré
            </span>
          </div>
          <div className="flex items-start gap-2">
            <span className="font-bold text-gray-800">Classe C ({report.summary.class_c_count} produits)</span>
            <span className="text-gray-700">
      ...
      <div className="text-xs text-gray-500 italic text-center py-2 border-t">
        ...
        Méthode Pareto ABC • Inspiré de Odoo, ERPNext, SAP
      </div>
    </div>
  )
}
