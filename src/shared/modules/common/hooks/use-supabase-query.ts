...
export function useSupabaseQuery<T = any>(
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (loadTime > 2000) {
        console.warn(`⚠️ SLO query dépassé: ${queryKey} ${Math.round(loadTime)}ms > 2000ms`)
      }
...
export function useSupabaseMutation<T = any>(
  mutationFn: (supabase: ReturnType<typeof createClient>, variables: any) => Promise<{ data: T | null; error: any }>
): MutationState<T> {
