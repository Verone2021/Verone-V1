...
export class GoogleMerchantSyncClient {
  ...
  constructor() {
    ...
    if (!this.accountId || !this.dataSourceId) {
      throw new Error(
        'Google Merchant configuration manquante (ACCOUNT_ID ou DATA_SOURCE_ID)'
      );
    }
  }
  ...
  private async waitForRateLimit(): Promise<void> {
    ...
    const delayNeeded =
      RATE_LIMIT_CONFIG.minDelayBetweenRequests - timeSinceLastRequest;
  ...
  private async fetchWithRetry(
    ...
    try {
      ...
      if (
        ...
        const retryDelay =
          RATE_LIMIT_CONFIG.retryDelayMs * Math.pow(2, attempt - 1); // Exponential backoff
   ...
   */
  async insertProduct(
    productInput: GoogleMerchantProductInput
  ): Promise<SyncResult> {
   ...
   */
  async insertProductsBatch(
    products: GoogleMerchantProductInput[]
