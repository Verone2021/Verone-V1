...
import { transformProductForGoogle, validateGoogleMerchantProduct } from './transformer'
...
interface ProductInsertRequest {
  parent: string
  productInput: any
  dataSource: string
}
...
export class GoogleMerchantClient {
   ...
   */
  async insertProduct(productData: any): Promise<GoogleMerchantApiResponse<ProductInputResponse>> {
    try {
      ...
      const insertRequest: ProductInsertRequest = {
        parent: getResourcePaths().account,
        productInput: transformedProduct,
        dataSource: this.dataSourcePath
      }

      // 4. Envoyer à Google
      const endpoint = `${getResourcePaths().productInputs}:insert`
      const result = await this.makeRequest<ProductInputResponse>(endpoint, 'POST', insertRequest)
   ...
   */
  async updateProduct(productData: any): Promise<GoogleMerchantApiResponse<ProductInputResponse>> {
    try {
  ...
  async deleteProduct(sku: string): Promise<GoogleMerchantApiResponse> {
    try {
      ...
      if (result.success) {
        console.log('[Google Merchant Client] Product deleted successfully:', { sku })
      }
   ...
   */
  async listProducts(pageSize = 100, pageToken?: string): Promise<GoogleMerchantApiResponse> {
    try {
   ...
   */
  async batchSyncProducts(products: any[]): Promise<GoogleMerchantApiResponse[]> {
    console.log(`[Google Merchant Client] Batch syncing ${products.length} products`)
    ...
    for (let i = 0; i < products.length; i += batchSize) {
      ...
      console.log(`[Google Merchant Client] Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(products.length / batchSize)}`)
      ...
      for (const product of batch) {
        ...
        if (GOOGLE_MERCHANT_CONFIG.rateLimit.requestsPerSecond > 0) {
          await new Promise(resolve => setTimeout(resolve, 1000 / GOOGLE_MERCHANT_CONFIG.rateLimit.requestsPerSecond))
        }
      }
    }
    ...
    console.log(`[Google Merchant Client] Batch sync completed: ${successCount} success, ${errorCount} errors`)
