 ...
 */
const AVAILABILITY_MAP: Record<string, 'in stock' | 'out of stock' | 'preorder'> = {
  'in_stock': 'in stock',
  'out_of_stock': 'out of stock',
  'preorder': 'preorder',
  'pret_a_commander': 'preorder',
  'coming_soon': 'preorder',
  'sourcing': 'out of stock',
  'discontinued': 'out of stock',
  'echantillon_a_commander': 'out of stock'
}
 ...
 */
function calculateSellingPrice(costPrice: number, marginPercentage: number | null): string {
...
function extractImageUrls(images: ProductImage[]): {
  ...
  const validImages = sortedImages.filter(img => img.public_url && img.public_url.trim().length > 0)
...
export function mapSupabaseToGoogleMerchant(
  ...
  if (!imageLink) {
    throw new Error(`Product ${product.sku}: Aucune image disponible (imageLink obligatoire)`)
  }
  ...
  const availability = AVAILABILITY_MAP[product.status || 'out_of_stock'] || 'out of stock'
