 ...
 */
function mapAvailability(status: VéroneProduct['status']): GoogleMerchantProduct['productAttributes']['availability'] {
  const mapping = {
    'in_stock': 'IN_STOCK',
 ...
 */
function mapCondition(condition: VéroneProduct['condition']): GoogleMerchantProduct['productAttributes']['condition'] {
  const mapping = {
    'new': 'NEW',
...
function extractPrimaryImage(product: VéroneProduct): string {
  ...
  return primaryImage?.public_url || fallbackImage?.public_url || `${GOOGLE_MERCHANT_CONFIG.productBaseUrl}/images/placeholder.jpg`
}
 ...
 */
function extractProductHighlights(product: VéroneProduct): string[] | undefined {
  if (!product.selling_points || product.selling_points.length === 0) return undefined

  return product.selling_points
    .slice(0, 3) // Google limite à 3 highlights
    .map(point => point.substring(0, GOOGLE_MERCHANT_CONFIG.validation.maxHighlightLength))
}

/**
 * Extrait les détails techniques pour Google
 */
function extractProductDetails(product: VéroneProduct): GoogleMerchantProduct['productAttributes']['productDetail'] | undefined {
...
function extractVariantAttributes(product: VéroneProduct) {
  ...
  return {
    ...
    material: variants.material || variants.materiau || variants.matiere || undefined,
    size: variants.size || variants.taille || extractSizeFromDimensions(product.dimensions) || undefined
  }
}
 ...
 */
function extractSizeFromDimensions(dimensions?: Record<string, any>): string | undefined {
 ...
 */
export function transformProductForGoogle(product: VéroneProduct): GoogleMerchantProduct {
  ...
  const googleProduct: GoogleMerchantProduct = {
    ...
    productAttributes: {
      // CHAMPS OBLIGATOIRES
      title: product.name.substring(0, GOOGLE_MERCHANT_CONFIG.validation.maxTitleLength),
      description: (product.description || product.name).substring(0, GOOGLE_MERCHANT_CONFIG.validation.maxDescriptionLength),
      ...
      // CHAMPS OPTIONNELS IMPORTANTS
      ...(extractProductHighlights(product) && { productHighlight: extractProductHighlights(product) }),
      ...(extractProductDetails(product) && { productDetail: extractProductDetails(product) }),
      ...(extractAdditionalImages(product).length > 0 && { additionalImageLink: extractAdditionalImages(product) }),
 ...
 */
export function validateGoogleMerchantProduct(product: GoogleMerchantProduct): { valid: boolean; errors: string[] } {
  const errors: string[] = []
  ...
  if (attrs.title.length > GOOGLE_MERCHANT_CONFIG.validation.maxTitleLength) {
    errors.push(`Titre trop long (max ${GOOGLE_MERCHANT_CONFIG.validation.maxTitleLength} chars)`)
  }

  if (attrs.description.length > GOOGLE_MERCHANT_CONFIG.validation.maxDescriptionLength) {
    errors.push(`Description trop longue (max ${GOOGLE_MERCHANT_CONFIG.validation.maxDescriptionLength} chars)`)
  }
