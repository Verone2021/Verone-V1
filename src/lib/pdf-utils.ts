...
export function generateSalesOrderPDF(
  ...
  // Calcul nom client
  const clientName = order.customer_type === 'organization'
    ? order.organisations?.name || 'Client professionnel'
    : `${order.individual_customers?.first_name || ''} ${order.individual_customers?.last_name || ''}`.trim() || 'Client particulier'
  ...
  doc.text(`Statut: ${statusLabels[order.status] || order.status}`, 25, yPosition + 20)

  if (order.expected_delivery_date) {
    doc.text(`Livraison prévue: ${formatDate(order.expected_delivery_date)}`, 25, yPosition + 26)
  }
  ...
  doc.text(`Type: ${order.customer_type === 'organization' ? 'Professionnel' : 'Particulier'}`, 115, yPosition + 20)
  ...
  autoTable(doc, {
    startY: yPosition,
    head: [['Produit', 'Référence', 'Qté', 'Prix Unit. HT', 'Remise', 'Total HT']],
    body: tableData,
    theme: 'striped',
    headStyles: {
      fillColor: [0, 0, 0],
