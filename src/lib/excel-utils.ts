...
export async function generateSalesOrdersExcel(
  ...
  orders.forEach(order => {
    const clientName = order.customer_type === 'organization'
      ...
      : `${order.individual_customers?.first_name || ''} ${order.individual_customers?.last_name || ''}`.trim() || 'N/A'

    const customerTypeLabel = order.customer_type === 'organization' ? 'Professionnel' : 'Particulier'
  ...
  statsData.forEach(([label, value]) => {
    ...
    // Styling valeur
    if (typeof value === 'number' && label !== 'Total Commandes' && label !== 'Commandes En Cours' && label !== 'Commandes Expédiées') {
