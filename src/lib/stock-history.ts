 ...
 */
export async function hasProductBeenInStock(productId: string): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('❌ Erreur lors de la vérification historique stock:', error)
...
export function useProductStockHistory() {
  const checkProductStockHistory = async (productId: string): Promise<boolean> => {
