...
export async function exportAgingReportToExcel(report: AgingReportData) {
  ...
  const overviewData = [
    ...
    ['Valeur totale stock', `${report.summary.total_value.toLocaleString('fr-FR')} €`],
    ['Âge moyen inventaire', `${report.summary.average_age_days} jours`],
    ['Stock vieilli (>90j)', `${report.summary.percent_over_90_days}%`],
    ['Valeur immobilisée', `${report.summary.immobilized_value.toLocaleString('fr-FR')} €`],
    [],
  ...
  const productsData = [
    ...
    [],
    ['Produit', 'SKU', 'Âge (jours)', 'Stock', 'Prix Unitaire', 'Valeur', 'Tranche', 'Dernier Mouvement'],
