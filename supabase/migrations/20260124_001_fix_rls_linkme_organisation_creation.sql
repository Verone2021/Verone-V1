-- =====================================================
-- MIGRATION: Fix RLS pour création organisations LinkMe
-- Date: 2026-01-24
-- Issue: Les affiliés LinkMe ne peuvent pas créer d'organisations client
--        car create_customer_organisation_for_affiliate utilise SECURITY INVOKER
-- Solution: Passer la fonction en SECURITY DEFINER pour contourner RLS
-- =====================================================
...
-- Recreate with SECURITY DEFINER (instead of SECURITY INVOKER)
CREATE OR REPLACE FUNCTION create_customer_organisation_for_affiliate(
  ...
  -- Geolocation parameters for TVA calculation
...
SECURITY: Uses SECURITY DEFINER to bypass RLS and allow authenticated LinkMe users
to create customer organisations.
...
DO $$
BEGIN
  RAISE NOTICE 'Migration 20260124_001: create_customer_organisation_for_affiliate switched to SECURITY DEFINER';
  RAISE NOTICE 'LinkMe affiliates can now create customer organisations';
END $$;
