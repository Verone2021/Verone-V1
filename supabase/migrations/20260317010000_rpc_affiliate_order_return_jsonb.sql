-- Migration: Change create_affiliate_order return type from UUID to JSONB
-- Reason: The hook use-submit-unified-order.ts needs order_number for email confirmation,
-- but anon users cannot SELECT from sales_orders (no RLS policy).
-- By returning {order_id, order_number} directly from the RPC, we eliminate the need
-- for a separate SELECT that fails for anonymous users on public order forms.

DROP FUNCTION IF EXISTS public.create_affiliate_order(uuid,uuid,text,uuid,jsonb,text,uuid,uuid,uuid,jsonb);
...
BEGIN
  ...
  -- Return both order_id and order_number so callers don't need a separate SELECT
