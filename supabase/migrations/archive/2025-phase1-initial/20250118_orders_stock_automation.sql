-- ================================================
-- GESTION AUTOMATIQUE DU STOCK PRÉVISIONNEL
...
BEGIN
    ...
    ELSIF v_payment_status = 'paid' AND OLD.payment_status != 'paid' THEN
    ...
    ELSIF v_new_status = 'cancelled' AND v_old_status != 'cancelled' THEN
        ...
        LOOP
            ...
            IF OLD.warehouse_exit_at IS NULL THEN
                UPDATE products
...
SELECT
    ...
    CASE
        WHEN p.stock_real + p.stock_forecasted_in - p.stock_forecasted_out < p.min_stock
        THEN 'critical'
        WHEN p.stock_real + p.stock_forecasted_in - p.stock_forecasted_out < p.reorder_point
        THEN 'low'
        ELSE 'ok'
    END AS stock_status
FROM products p;
