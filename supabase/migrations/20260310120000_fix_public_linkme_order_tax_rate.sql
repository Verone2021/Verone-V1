-- ============================================================================
-- Migration: Fix create_public_linkme_order tax_rate handling
-- Date: 2026-03-10
-- Bug: tax_rate not explicitly set on sales_orders (relies on DB default).
--      Also, total calculation hardcodes /1.2 which is wrong for exports (BE).
-- Fix: Look up organisation.default_vat_rate or country, set tax_rate explicitly.
-- ============================================================================

-- Drop existing function (parameter names changed)
DROP FUNCTION IF EXISTS public.create_public_linkme_order(uuid, uuid, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb);
...
BEGIN
  ...
  -- =========================================
  -- 9. CREATE SALES_ORDER (with explicit tax_rate)
  -- =========================================
