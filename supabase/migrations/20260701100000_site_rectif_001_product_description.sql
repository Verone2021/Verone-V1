-- [SITE-RECTIF-001] Fiche produit : afficher la vraie description
--
-- Contexte : sur veronecollections.fr, la fiche produit n'affiche aucune
-- description pour la quasi-totalité du catalogue. La RPC get_site_internet_products
-- renvoyait la colonne products.description (remplie sur seulement 6/178 produits)
-- alors que le contenu rédigé vit dans description_long (157/178) et
-- description_short (160/178).
--
-- Correctif : le champ `description` renvoyé par la RPC tombe désormais en
-- cascade sur description_long puis description_short quand description est vide.
-- La SIGNATURE de la fonction est INCHANGÉE (mêmes colonnes, mêmes types) :
-- aucune régénération de types Supabase n'est nécessaire (pas de drift).
--
-- Seule ligne modifiée par rapport à la définition en production :
--   AVANT : p.description, p.technical_description, ...
--   APRÈS : COALESCE(NULLIF(TRIM(p.description), ''), p.description_long, p.description_short), p.technical_description, ...
-- Le LEFT(p.description, 160) utilisé pour le seo_meta_description reste
-- volontairement sur la colonne d'origine.
