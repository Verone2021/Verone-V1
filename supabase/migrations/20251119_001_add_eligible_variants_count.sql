-- Migration: Ajouter eligible_variants_count + variant_group_id dans get_site_internet_products()
-- Date: 2025-11-19
-- Description: Compter UNIQUEMENT variantes éligibles (publiées) au lieu de toutes les variantes
-- Use Case: Si variant_group a 16 variantes mais seulement 2 publiées → eligible_variants_count: 2

DROP FUNCTION IF EXISTS get_site_internet_products();

CREATE OR REPLACE FUNCTION get_site_internet_products()
RETURNS TABLE (
  -- Champs existants (31 colonnes - ajout variant_group_id + eligible_variants_count)
  product_id UUID,
  sku TEXT,
  name TEXT,
  slug TEXT,
  ...
  variant_group_id UUID, -- ✨ NOUVEAU (requis pour récupérer variantes sœurs)
  eligible_variants_count INTEGER, -- ✨ NOUVEAU (compte uniquement variantes éligibles)
  is_eligible BOOLEAN,
...
BEGIN
  ...
  SELECT
    ...
    -- ✨ NOUVEAU : variant_group_id (pour récupérer variantes sœurs)
    p.variant_group_id,

    -- ✨ NOUVEAU : Count UNIQUEMENT variantes éligibles (publiées sur site internet)
    CASE
      WHEN p.variant_group_id IS NOT NULL THEN
        (
...
COMMENT ON FUNCTION get_site_internet_products() IS
  ...
  33 champs retournés (31 champs + variant_group_id + eligible_variants_count).
  ✅ Fix 2025-11-19: Ajout variant_group_id + eligible_variants_count pour filtrer variantes éligibles uniquement';
