-- Fix: unified_status = 'matched' doit aussi couvrir les transactions
-- manual_matched et auto_matched qui n'ont pas de matched_document_id
-- (cas : rapprochement lié à une commande directement, pas à un document)
-- 5 des 6 rapprochements manuels 2026 avaient matched_document_id = null
-- → classés 'to_process' au lieu de 'matched' dans v_transactions_unified
...
SELECT
  ...
  CASE
    ...
    WHEN bt.matched_document_id IS NOT NULL
      OR bt.matching_status IN ('manual_matched'::matching_status, 'auto_matched'::matching_status)
      THEN 'matched'::text
