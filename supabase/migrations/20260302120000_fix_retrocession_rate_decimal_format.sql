-- ============================================================================
-- Migration: Fix retrocession_rate format (percentage → decimal) for catalog items
-- Date: 2026-03-02
--
-- PROBLEM: The RPC create_affiliate_order stored retrocession_rate as a percentage
--          value (e.g., 15 for 15%) instead of decimal (0.15).
--          The BEFORE trigger calculate_retrocession_amount expects decimal for
--          catalog products:
--            retrocession_amount = unit_price_ht * quantity * retrocession_rate
--          With 15 stored: 21.39 × 15 = 320.85 (×100 too high!)
--          With 0.15 stored: 21.39 × 0.15 = 3.21 (correct)
--
-- FIX 1: Convert existing records where retrocession_rate > 1 (percentage format)
--         The BEFORE trigger auto-recalculates retrocession_amount correctly.
--
-- FIX 2: Update the RPC to store margin_rate / 100 (decimal format).
-- ============================================================================

-- -----------------------------------------------
-- FIX 1: Existing data (idempotent: WHERE rate > 1)
-- -----------------------------------------------
-- Note: The BEFORE UPDATE trigger (trg_calculate_retrocession) will automatically
-- recalculate retrocession_amount using: unit_price_ht * quantity * new_rate
UPDATE sales_order_items
SET retrocession_rate = retrocession_rate / 100
WHERE retrocession_rate > 1
  AND linkme_selection_item_id IS NOT NULL;

-- -----------------------------------------------
-- FIX 2: RPC create_affiliate_order - store decimal format
-- -----------------------------------------------
...
BEGIN
  ...
  --   retrocession_amount = unit_price_ht * quantity * retrocession_rate
  -- The BEFORE INSERT trigger (trg_calculate_retrocession) will auto-calculate
  -- retrocession_amount, so we only need to set retrocession_rate correctly.
