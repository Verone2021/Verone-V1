-- ============================================
-- Migration: Add billing/shipping addresses to get_linkme_orders
-- Date: 2026-01-06
-- Description: Ajoute les adresses de facturation/livraison et dates de livraison
--              pour le modal de detail dans l'app LinkMe
-- ============================================
...
RETURNS TABLE (
  ...
  customer_email TEXT,
  customer_phone TEXT,
  -- NEW: Adresses structurees
  billing_address JSONB,
  shipping_address JSONB,
  -- NEW: Dates de livraison
  desired_delivery_date DATE,
  confirmed_delivery_date DATE,
  -- NEW: Contact facturation
  billing_name TEXT,
  billing_email TEXT,
  billing_phone TEXT,
  -- Existing fields
...
LANGUAGE sql STABLE SECURITY INVOKER AS $$
  ...
  WHERE
    ...
    -- Via selection OU via commissions OU via created_by_affiliate_id
    (
...
'RPC pour recuperer les commandes LinkMe avec adresses et dates.
- p_affiliate_id = NULL : toutes les commandes (mode CMS)
- p_affiliate_id = UUID : commandes de l affilie
Champs ajoutes le 2026-01-06:
- billing_address, shipping_address (JSONB depuis sales_orders)
- desired_delivery_date, confirmed_delivery_date (depuis sales_order_linkme_details)
- billing_name, billing_email, billing_phone (depuis sales_order_linkme_details)';
