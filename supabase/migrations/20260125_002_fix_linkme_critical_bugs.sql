-- ============================================================================
-- Migration: Fix LinkMe Critical Bugs
-- Date: 2026-01-25
-- Description: Corrige les bugs critiques identifiés par l'audit LinkMe
--
-- Bugs traités:
--   #1: RLS optimisée pour sales_orders (wrapper SELECT auth.uid())
--   #2: Suppression politique temp d'audit conflictuelle
--   #3: Formule retrocession_amount corrigée (margin_rate / 100)
-- ============================================================================

-- ============================================
-- BUG #1 & #2: Nettoyage et optimisation RLS
-- ============================================

-- Supprimer la politique temp d'audit qui peut causer des conflits
DROP POLICY IF EXISTS "temp_audit_all_authenticated_can_read_linkme_orders" ON sales_orders;

-- Recréer politique staff optimisée avec wrapper (SELECT auth.uid())
...
CREATE POLICY "Staff can view all sales_orders" ON sales_orders
  ...
  USING (
    EXISTS (
      ...
      AND up.role IN ('owner', 'admin', 'sales', 'catalog_manager', 'partner_manager')
    )
  );

-- Politique pour affiliés LinkMe: voir leurs propres commandes
DROP POLICY IF EXISTS "LinkMe affiliates can view their orders" ON sales_orders;
CREATE POLICY "LinkMe affiliates can view their orders" ON sales_orders
  FOR SELECT
  USING (
...
-- ============================================
-- Optimiser RLS sales_order_items
-- ============================================
...
CREATE POLICY "LinkMe affiliates can view their order items" ON sales_order_items
  FOR SELECT
  USING (
    sales_order_id IN (
      SELECT so.id FROM sales_orders so
      WHERE so.created_by_affiliate_id IN (
        SELECT la.id FROM linkme_affiliates la
        JOIN user_app_roles uar ON (
          (uar.enseigne_id IS NOT NULL AND uar.enseigne_id = la.enseigne_id)
          OR (uar.organisation_id IS NOT NULL AND uar.organisation_id = la.organisation_id)
...
-- ============================================
-- Optimiser RLS sales_order_linkme_details
-- ============================================
...
CREATE POLICY "LinkMe affiliates can view their linkme details" ON sales_order_linkme_details
  FOR SELECT
  USING (
    sales_order_id IN (
      SELECT so.id FROM sales_orders so
...
-- ============================================
-- BUG #3: Corriger formule retrocession_amount
-- margin_rate est stocké en pourcentage (ex: 15.00)
-- Formule correcte: base_price_ht * (margin_rate / 100) * quantity
-- ============================================
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO sales_order_items (
      ...
      -- FIX BUG #3: margin_rate est en pourcentage, diviser par 100
...
SELECT 'Migration 20260125_002_fix_linkme_critical_bugs applied successfully' AS status;
