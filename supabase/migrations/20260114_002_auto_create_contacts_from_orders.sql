-- ============================================================================
-- Migration: Auto-create contacts from LinkMe orders
-- Date: 2026-01-14
-- Description: Automatically create contacts in CRM when creating public orders
-- ============================================================================
...
CREATE OR REPLACE FUNCTION create_public_linkme_order(
  ...
  p_organisation JSONB,   -- {existing_id} OU {is_new, trade_name, city, latitude, longitude, ...}
...
BEGIN
  ...
  -- =========================================
  -- 4. CRÉER CONTACTS AUTOMATIQUEMENT
  -- =========================================

  -- 4.1 Contact Propriétaire (Contact Principal)
  IF (p_owner->>'email') IS NOT NULL AND (p_owner->>'name') IS NOT NULL THEN
    INSERT INTO contacts (
      ...
      v_customer_id,
      -- Extraction prénom/nom depuis le nom complet
  ...
  -- 4.2 Contact Facturation (si différent du propriétaire)
  IF (p_billing->>'contact_source') = 'custom'
  ...
  -- =========================================
  -- 9. CRÉER SALES_ORDER_LINKME_DETAILS
  -- =========================================
...
AUTO-CRÉATION CONTACTS (2026-01-14):
- Crée automatiquement le contact propriétaire comme contact principal (is_primary_contact = true)
- Si billing custom, crée le contact facturation (is_billing_contact = true)
- Gère les doublons via ON CONFLICT (email unique par organisation)
- Extraction automatique prénom/nom depuis nom complet
- Retourne owner_contact_id et billing_contact_id

GEOLOCALISATION (2026-01-14):
- Accepte maintenant latitude/longitude dans p_organisation
- Stocke les coordonnées GPS pour affichage carte et recherche géographique
...
- p_billing: JSONB {contact_source, delivery_date, mall_form_required, name, email, phone}

RETOURNE:
- Succès: {success: true, order_id, order_number, customer_id, total_ttc, owner_contact_id, billing_contact_id}
