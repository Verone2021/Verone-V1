-- Migration: Fix get_site_internet_products() - Filtrer UNIQUEMENT produits éligibles
-- Date: 2025-11-18
-- Description: Ajouter WHERE clause pour ne retourner QUE les produits éligibles (is_eligible = TRUE)
-- Bug Fix: Actuellement TOUS les produits sont retournés (16/16), devrait retourner seulement 7 éligibles

DROP FUNCTION IF EXISTS get_site_internet_products();

CREATE OR REPLACE FUNCTION get_site_internet_products()
RETURNS TABLE (
  -- Champs existants (19 colonnes - ajout discount_rate)
  product_id UUID,
  sku TEXT,
  name TEXT,
  slug TEXT,
...
COMMENT ON FUNCTION get_site_internet_products() IS
  'RPC Site Internet: retourne UNIQUEMENT les produits éligibles (status=active, published=true, slug présent, prix>0, images présentes).
  30 champs retournés (18 existants + 12 nouveaux).
  ✅ Fix 2025-11-18: Ajout WHERE clause pour filtrer is_eligible=TRUE';
