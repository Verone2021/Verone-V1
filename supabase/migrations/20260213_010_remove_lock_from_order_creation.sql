-- Migration 010: Remove price locking from order creation functions
--
-- With migration 009 (auto lock on validation), prices are now locked/unlocked
-- via the order lifecycle (draft -> validated -> draft). Draft orders should
-- always reflect current catalog prices, so we remove the locked columns
-- from the INSERT in both creation functions.
--
-- Changes:
--   - create_public_order(): Remove base_price_ht_locked, selling_price_ht_locked, price_locked_at from INSERT
--   - create_affiliate_order(): Remove base_price_ht_locked, selling_price_ht_locked, price_locked_at from INSERT

-- ============================================
-- Recreate create_public_order WITHOUT locked columns
-- ============================================
...
BEGIN
  ...
  -- 6. Creer les lignes de commande (sans locked columns - lock happens at validation)
...
-- ============================================
-- Recreate create_affiliate_order WITHOUT locked columns
-- ============================================
