-- Add stock_status to get_site_internet_products RPC
-- Needed for: feed XML (out of stock availability) + site internet badge "Rupture de stock"
-- Copy of 20260318160000 with stock_status added to RETURNS TABLE and SELECT
...
RETURNS TABLE (
  ...
  has_variants BOOLEAN, variants_count INTEGER, variant_group_id UUID, eligible_variants_count INTEGER,
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(cpm.custom_title::TEXT, cpm.metadata->>'seo_title', p.meta_title::TEXT, p.name::TEXT) AS seo_title,
    ...
    p.is_published_online AS is_published, p.publication_date,
    ...
    COALESCE((SELECT ARRAY(SELECT jsonb_array_elements_text(cpm.custom_selling_points)) WHERE cpm.custom_selling_points IS NOT NULL AND jsonb_typeof(cpm.custom_selling_points) = 'array'), (SELECT ARRAY(SELECT jsonb_array_elements_text(p.selling_points)) WHERE p.selling_points IS NOT NULL AND jsonb_typeof(p.selling_points) = 'array'), ARRAY[]::TEXT[]) AS selling_points,
