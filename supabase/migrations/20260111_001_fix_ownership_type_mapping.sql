-- ============================================================================
-- Migration: Fix ownership_type mapping in create_public_linkme_order RPC
-- Date: 2026-01-11
-- Description: Maps ownerType from forms to ownership_type column in organisations
--              'propre' -> 'succursale', 'franchise' -> 'franchise'
-- ============================================================================

-- ============================================
-- 1. UPDATE create_public_linkme_order RPC
-- ============================================
-- Add ownership_type when creating new organisations
...
BEGIN
  ...
  ELSE
    ...
    INSERT INTO organisations (
      ...
      enseigne_id,
      type,
      ownership_type  -- NOUVEAU: mapper ownership_type
    ) VALUES (
      ...
      v_enseigne_id,
      'customer',
      v_ownership_type  -- NOUVEAU: inclure ownership_type
    )
  ...
  -- =========================================
  -- 6. CRÉER SALES_ORDER (sans totaux, ils seront mis à jour après)
  -- =========================================
...
-- ============================================
-- 2. BACKFILL: Migrate existing data from sales_order_linkme_details
-- ============================================
-- Update organisations that have orders with owner_type set but no ownership_type

UPDATE organisations o
SET ownership_type = CASE
  WHEN d.owner_type = 'propre' THEN 'succursale'::organisation_ownership_type
  WHEN d.owner_type = 'franchise' THEN 'franchise'::organisation_ownership_type
  ELSE NULL
END
FROM sales_orders so
JOIN sales_order_linkme_details d ON d.sales_order_id = so.id
WHERE so.customer_id = o.id
  AND o.ownership_type IS NULL
  AND d.owner_type IS NOT NULL
  AND d.owner_type IN ('propre', 'franchise');
...
MAPPING ownership_type (depuis 2026-01-11):
