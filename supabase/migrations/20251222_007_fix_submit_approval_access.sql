-- =====================================================
-- MIGRATION: Fix submit_affiliate_product_for_approval avec meilleurs messages d'erreur
-- Date: 2025-12-22
-- Description: Ajoute des messages d'erreur explicites pour diagnostiquer
--              les problemes d'acces lors de la soumission
-- =====================================================
...
BEGIN
  ...
  IF NOT v_has_access THEN
    RAISE EXCEPTION 'Access denied for user % on product % (affiliate org: %, enseigne: %)',
      v_user_id, p_product_id, v_product.affiliate_organisation_id, v_product.affiliate_enseigne_id;
  END IF;

  -- Check product is in draft status
  IF v_product.affiliate_approval_status != 'draft' THEN
    RAISE EXCEPTION 'Product must be in draft status to submit for approval (current: %)',
      v_product.affiliate_approval_status;
  END IF;
...
Supporte les enseignes ET les organisations independantes.
Retourne des messages d''erreur explicites pour le debug.';
