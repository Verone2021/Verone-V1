-- Migration: Hotfix condition ligne 39 (stock = 0 avec min_stock = 0)
-- Bug: Ligne 39 utilise <= au lieu de <
-- Impact: Faux positifs pour produits avec stock = 0 et min_stock = 0
...
BEGIN
  ...
  -- ✅ CONDITION 2 CORRIGÉE : Stock NÉGATIF uniquement (< au lieu de <=)
  -- Alerte UNIQUEMENT si stock prévisionnel strictement négatif
  -- PAS d'alerte si stock = 0 et min_stock = 0 (acceptable)
  ELSIF v_forecasted_stock < 0 THEN
    v_alert_type := 'out_of_stock';
    v_alert_priority := 3;
    v_shortage := COALESCE(NEW.min_stock, 0);

  -- ✅ CONDITION 3 : Stock sous seuil minimum (< au lieu de <=)
...
HOTFIX 2025-11-19 (v2) : Correction conditions lignes 39 et 43 (<= → <).
Stock prévisionnel = stock_real - stock_forecasted_out + stock_forecasted_in.
Alertes créées SI : stock < 0 OU (stock <= 0 ET commandes clients) OU stock < min_stock.';
