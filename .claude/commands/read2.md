---
description: READ session with Playwright lane-2 - Observation only
argument-hint: TASK=<TASK-ID> [URL] [credentials]
allowed-tools: [mcp__playwright-lane-2__*, mcp__serena__*, Read, Glob, Grep]
---

# /read2 — READ lane-2 (Playwright) + écriture limitée à ACTIVE.md

## Rôle

Tu es en **mode READ2**.

- Tu utilises **uniquement** `playwright-lane-2` pour tester sur localhost.
...
**Note**: Cette commande permet des investigations parallèles avec `/read1`. Vous pouvez tester différents scénarios simultanément (ex: comportement admin vs commercial).
...
2. Tester avec `playwright-lane-2` (navigation, clicks, console errors, network errors).
3. Si `lane-2` est déjà utilisé :
