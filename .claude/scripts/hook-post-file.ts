...
/**
 * PostToolUse Hook - Auto-format TypeScript files
 *
 * Adapted from AIBlueprint for Vérone Back Office
 *
 * Runs after Edit/Write/MultiEdit to:
 * 1. Format with Prettier
 * 2. Fix with ESLint --fix
...
async function main() {
  ...
  if (filePath.includes('supabase.ts') || filePath.includes('.d.ts')) {
    log(`Skipping ${filePath}: generated file`);
    process.exit(0);
  }
