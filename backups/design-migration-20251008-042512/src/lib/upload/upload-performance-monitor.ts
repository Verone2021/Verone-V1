...
export class UploadPerformanceMonitor {
  ...
  private cleanupOldMetrics(): void {
    ...
    console.log(`🧹 Nettoyage: ${this.sessionMetrics.length} métriques conservées`)
  }
   ...
   */
  startUploadMonitoring(uploadId: string, context: {
    fileType: string
    originalSize: number
    userSegment: string
    clientType: 'b2b' | 'b2c' | 'prospect'
   ...
   */
  recordOptimizationTime(uploadId: string, optimizationTimeMs: number, details: {
    variantsGenerated: number
    compressionRatio: number
    ...
    metrics.size.totalSaved = metrics.size.originalBytes - details.optimizedSize

    // Vérifier seuils optimisation
    this.checkOptimizationThresholds(uploadId, metrics)

    console.log(`⏱️ Optimisation: ${optimizationTimeMs}ms, compression: ${details.compressionRatio}%`)
  }

  /**
   * 🌐 Enregistrer métriques réseau
   */
  recordNetworkMetrics(uploadId: string, networkData: {
    speedKbps: number
    latencyMs: number
    retryCount: number
    chunkingUsed: boolean
   ...
   */
  completeUploadMonitoring(uploadId: string, results: {
    uploadStrategy: string
    ...
    metrics.timing.totalTime = metrics.timing.endTime - metrics.timing.startTime

    // Finaliser qualité
    metrics.context.uploadStrategy = results.uploadStrategy
    metrics.quality.successRate = (results.successfulUploads / results.totalUploads) * 100
    ...
    console.log(`🎉 Monitoring terminé: ${uploadId}, score: ${report.score.overall}/100`)
   ...
   */
  recordUploadFailure(uploadId: string, error: {
    type: string
    message: string
    critical: boolean
    phase: string
   ...
   * 🔍 Vérifier seuils optimisation
   */
  private checkOptimizationThresholds(uploadId: string, metrics: UploadPerformanceMetrics): void {
   ...
   * 🌐 Vérifier seuils réseau
   */
  private checkNetworkThresholds(uploadId: string, metrics: UploadPerformanceMetrics): void {
    // Vitesse upload
    if (metrics.network.uploadSpeedKbps < this.thresholds.speedKbpsMin) {
      console.log(`🐌 Vitesse lente: ${metrics.network.uploadSpeedKbps}Kbps < ${this.thresholds.speedKbpsMin}Kbps`)
    }
   ...
   * 📊 Générer rapport complet
   */
  private generatePerformanceReport(metrics: UploadPerformanceMetrics): UploadPerformanceReport {
   ...
   * 🚨 Générer alertes
   */
  private generateAlerts(metrics: UploadPerformanceMetrics): PerformanceAlert[] {
    const alerts: PerformanceAlert[] = []
   ...
   */
  private generateRecommendations(metrics: UploadPerformanceMetrics): PerformanceRecommendation[] {
    ...
    if (metrics.timing.optimizationTime > 3000) {
      recommendations.push({
        ...
        priority: 'medium',
        suggestion: 'Activer compression aggressive pour réduire temps optimisation',
   ...
   */
  private calculatePerformanceScore(metrics: UploadPerformanceMetrics): PerformanceScore {
    // Calcul scores composants (0-100)
    const speedScore = Math.min(100, (metrics.network.uploadSpeedKbps / this.thresholds.speedKbpsMin) * 100)
    const compressionScore = Math.min(100, (metrics.size.compressionRatio / this.thresholds.compressionRatioMin) * 100)
    const reliabilityScore = metrics.quality.successRate
    const efficiencyScore = Math.max(0, 100 - (metrics.timing.totalTime / this.thresholds.totalTimeMs) * 100)
  ...
  private calculateTrends(): PerformanceTrends {
    ...
    const averageSpeed = recentMetrics.reduce((sum, m) => sum + m.network.uploadSpeedKbps, 0) / recentMetrics.length
    const successRate = recentMetrics.reduce((sum, m) => sum + m.quality.successRate, 0) / recentMetrics.length
    const compressionEfficiency = recentMetrics.reduce((sum, m) => sum + m.size.compressionRatio, 0) / recentMetrics.length
    ...
    return {
      ...
      alerts: {
        critical: recentMetrics.filter(m => m.quality.criticalErrors.length > 0).length,
  ...
  private trackAnalytics(metrics: UploadPerformanceMetrics): void {
    ...
    gdprAnalytics.trackPerformance('image_upload_complete', metrics.timing.totalTime)

    // Métriques business si consent
    gdprAnalytics.trackBusinessMetric('upload_performance_score', metrics.quality.successRate, {
  ...
  private calculateAverageComparison(currentScore: number): number {
    ...
    const averageScore = this.sessionMetrics.reduce((sum, metrics) => {
...
export const UploadMonitoringUtils = {
  ...
  formatMetrics(metrics: UploadPerformanceMetrics): string {
    return `Upload ${metrics.uploadId}: ${Math.round(metrics.timing.totalTime)}ms, ` +
  ...
  generateSummary(report: UploadPerformanceReport): string {
    ...
    return `Performance ${score.grade}: ${score.overall}/100 ` +
