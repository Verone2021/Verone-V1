...
test.describe('Dashboard Stocks - Séparation Visuelle Renforcée', () => {
  ...
  test('section STOCK RÉEL : affiche emoji ✓ dans le titre', async ({
    page,
  }) => {
  ...
  test('section STOCK RÉEL : affiche badge "Mouvements Effectués"', async ({
    page,
  }) => {
  ...
  test('section STOCK PRÉVISIONNEL : affiche emoji ⏱ dans le titre', async ({
    page,
  }) => {
  ...
  test('section STOCK PRÉVISIONNEL : affiche badge "Commandes En Cours"', async ({
    page,
  }) => {
  ...
  test('section STOCK PRÉVISIONNEL : affiche texte "INFORMATIF uniquement"', async ({
    page,
  }) => {
  ...
  test('séparation visuelle : espacement vertical entre les 2 sections', async ({
    page,
  }) => {
  ...
  test('widget Mouvements 7 Derniers Jours affiche données', async ({
    page,
  }) => {
    const widget = page.getByRole('heading', {
      name: 'Mouvements 7 Derniers Jours',
    });
  ...
  test('widget Stock Prévisionnel affiche commandes', async ({ page }) => {
    ...
    await expect(
      page.getByRole('button', { name: /Entrées Prévues/ })
    ).toBeVisible();
    await expect(
      page.getByRole('button', { name: /Sorties Prévues/ })
    ).toBeVisible();
  });
