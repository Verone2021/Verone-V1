...
test.describe('Page Inventaire - Tests de Régression', () => {
  ...
  test('navigation vers autres pages stocks fonctionne', async ({ page }) => {
    ...
    const mouvementsLink = page
      .getByRole('link', { name: /mouvements/i })
      .first();
  ...
  test("tableau inventaire s'affiche si produits présents", async ({
    page,
  }) => {
    // Attendre un tableau ou un message "Aucun produit"
    const hasTable = (await page.locator('table').count()) > 0;
    const hasEmptyMessage =
      (await page.getByText(/aucun produit/i).count()) > 0;
